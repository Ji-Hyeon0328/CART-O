...
parser.add_argument("--enable_b84_post_touchdown_settle_glide", action="store_true", default=True)
parser.add_argument("--b84_settle_glide_steps", type=int, default=520)
parser.add_argument("--b84_dt", type=float, default=0.02)
parser.add_argument("--b84_ref_follow_gain", type=float, default=0.75)
parser.add_argument("--b84_max_ref_step", type=float, default=0.0025)
parser.add_argument("--b84_max_ref_offset", type=float, default=0.22)
parser.add_argument("--b84_xy_kp", type=float, default=4.0)
parser.add_argument("--b84_xy_kd", type=float, default=2.0)
parser.add_argument("--b84_max_base_acc_lin", type=float, default=0.65)
parser.add_argument("--b84_rp_kp", type=float, default=85.0)
parser.add_argument("--b84_rp_kd", type=float, default=14.0)
parser.add_argument("--b84_z_kp", type=float, default=80.0)
parser.add_argument("--b84_stance_acc_weight", type=float, default=160.0)
parser.add_argument("--b84_posture_max_tau", type=float, default=10.0)
parser.add_argument("--b84_activate_on_legs", type=str, default="LH,RH")
parser.add_argument("--b84_print", action="store_true", default=True)
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B84 POST-TOUCHDOWN SETTLE-GLIDE + B83/B82/B81/B80 recovery planner")
    ...
    b84_settle_glide_count = 0
    b84_glide_ref_xy = None
    b84_glide_start_xy = None
    b84_glide_reason = None
    b84_glide_leg = None
    ...
    for step in range(total_steps):
        ...
        if args.enable_canonical_gait_schedule and args.canonical_gait_disable_event_overrides:
                ...
                if canonical_latched_swing_leg is not None:
                    ...
                    if canonical_latched_swing_elapsed < duration:
                        ...
                        if _b82_stand_lock:
                            ...
                            try:
                                _b84_legs = [x.strip().upper() for x in str(getattr(args, "b84_activate_on_legs", "LH,RH")).split(",") if x.strip()]
                                _b84_leg_name = ID_TO_LEG[int(canonical_latched_swing_leg)].upper()
                                if bool(getattr(args, "enable_b84_post_touchdown_settle_glide", True)) and _b84_leg_name in _b84_legs:
                                    b84_settle_glide_count = max(int(b84_settle_glide_count), int(getattr(args, "b84_settle_glide_steps", 520)))
                                    b84_glide_ref_xy = x_hat[:, 0:2].detach().clone()
                                    b84_glide_start_xy = x_hat[:, 0:2].detach().clone()
                                    b84_glide_reason = str(b77_reason)
                                    b84_glide_leg = _b84_leg_name
                                    if bool(getattr(args, "b84_print", True)):
                                        print("[B84 SETTLE-GLIDE START]",
                                              "step=", step,
                                              "closed_leg=", _b84_leg_name,
                                              "steps=", int(b84_settle_glide_count),
                                              "base_xy=", x_hat[0,0:2].detach().cpu().numpy(),
                                              "vxy=", x_hat[0,6:8].detach().cpu().numpy(),
                                              "reason=", b84_glide_reason)
                            except Exception as _b84_e:
                                if bool(getattr(args, "b84_print", True)):
                                    print("[B84 SETTLE-GLIDE START ERROR]", "step=", step, "error=", repr(_b84_e))
                            canonical_all_stance_gap_count = max(
                                int(canonical_all_stance_gap_count),
            ...
            if bool(getattr(args, "enable_b78_true_all_stance_recovery", True)) and int(b78_recovery_count) > 0:
                b84_active_now = bool(getattr(args, "enable_b84_post_touchdown_settle_glide", True)) and int(b84_settle_glide_count) > 0
                if b84_active_now:
                    ...
                    if b84_glide_ref_xy is None:
                        b84_glide_ref_xy = x_hat[:, 0:2].detach().clone()
                    if b84_glide_start_xy is None:
                        b84_glide_start_xy = b84_glide_ref_xy.detach().clone()
                    vxy = x_hat[:, 6:8].detach()
                    dt84 = float(getattr(args, "b84_dt", 0.02))
                    follow = float(getattr(args, "b84_ref_follow_gain", 0.75))
                    raw_delta = vxy * dt84 * follow
                    max_step84 = float(getattr(args, "b84_max_ref_step", 0.0025))
                    ...
                    new_ref_xy = b84_glide_ref_xy + delta
                    max_off84 = float(getattr(args, "b84_max_ref_offset", 0.22))
                    off = new_ref_xy - b84_glide_start_xy
                    off_norm = torch.linalg.norm(off, dim=1, keepdim=True).clamp_min(1e-9)
                    off_scale = torch.clamp(max_off84 / off_norm, max=1.0)
                    b84_glide_ref_xy = b84_glide_start_xy + off * off_scale
                    base_ref = x_hat[:, 0:6].detach().clone()
                    base_ref[:, 0:2] = b84_glide_ref_xy.detach().clone()
                    base_ref[:, 2] = float(getattr(args, "b79_recovery_height", args.height_ref))
                    base_ref[:, 3:6] = 0.0
                    prev_base_ref = base_ref.detach().clone()
                    if bool(getattr(args, "b84_print", True)) and (step % max(args.print_every, 1) == 0 or int(b84_settle_glide_count) == int(getattr(args, "b84_settle_glide_steps", 520))):
                        print("[B84 SETTLE-GLIDE]",
                              "step=", step,
                              "remaining=", int(b84_settle_glide_count),
                              "leg=", b84_glide_leg,
                ...
                if bool(getattr(args, "b78_print", True)) and (step % max(args.print_every, 1) == 0 or int(b78_recovery_count) == int(getattr(args, "b78_recovery_steps", 360))):
                    print("[B78 TRUE ALL-STANCE RECOVERY]",
                          ...
                          "b84_settle_glide=", b84_active_now)
        ...
        if bool(getattr(args, "enable_b78_true_all_stance_recovery", True)) and int(b78_recovery_count) > 0:
            ...
            if int(b84_settle_glide_count) > 0:
                b84_settle_glide_count = max(0, int(b84_settle_glide_count) - 1)
        ...
        if (bool(getattr(args, "enable_b79_latched_recovery_posture", True))
                and int(b78_recovery_count) > 0):
            wbc_cfg_active = copy.copy(wbc_cfg_active)
            if bool(getattr(args, "enable_b84_post_touchdown_settle_glide", True)) and int(b84_settle_glide_count) > 0:
                # B84: avoid hard xy braking. Use mild xy gains / capped linear accel,
                # but keep attitude and height stabilization relatively strong.
                wbc_cfg_active.kp_base_xy = float(getattr(args, "b84_xy_kp", 4.0))
                wbc_cfg_active.kd_base_xy = float(getattr(args, "b84_xy_kd", 2.0))
                wbc_cfg_active.max_base_acc_lin = float(getattr(args, "b84_max_base_acc_lin", 0.65))
                wbc_cfg_active.kp_base_z = float(getattr(args, "b84_z_kp", 80.0))
                wbc_cfg_active.kp_base_rp = float(getattr(args, "b84_rp_kp", 85.0))
                wbc_cfg_active.kd_base_rp = float(getattr(args, "b84_rp_kd", 14.0))
                wbc_cfg_active.w_stance_acc = float(getattr(args, "b84_stance_acc_weight", 160.0))
            else:
        ...
        if ('b79_recovery_q_cmd' in locals() and b79_recovery_q_cmd is not None
            ...
            _b84_tau_cap = float(getattr(args, "b84_posture_max_tau", args.b79_posture_max_tau)) if int(b84_settle_glide_count) > 0 else float(args.b79_posture_max_tau)
