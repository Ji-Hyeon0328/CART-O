...
#
# B8-bj: freeze base during swing and foothold-lock recovery.
#
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B8-bj freeze base during swing and foothold-lock recovery")
...
def print_debug(step, phase, profile, phase_step, x_hat, base_ref, trial_out, ref_mask, contact_mask,
    ...
    print("\n" + "=" * 150)
    print(f"[B8-bj FREEZE-BASE-FOOTHOLD-LOCK] step={step}")
    print("=" * 150)
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B8-bj freeze base during swing and foothold-lock recovery")
    ...
    for step in range(total_steps):
        ...
        freeze_base_now = (
            (args.freeze_base_ref_during_swing and phase in ["lift", "hold_lift", "lower"] and swing_enabled)
            or (args.freeze_base_ref_during_touchdown_recovery and (touchdown_gate_active or freeze_step_key_for_touchdown))
        )
        if freeze_base_now:
        ...
        if phase in ["shift", "settle", "recenter", "warmup", "done"] or late_touchdown or (args.recenter_require_safe_for_next and step_idx >= 1 and not recenter_safe_seen):
            # pre-shift / settle / recenter / warmup / late-touchdown keep all contacts
            swing_enabled = False
