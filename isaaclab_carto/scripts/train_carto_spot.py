...
def get_action_std(step: int, init_std: float = 0.10, final_std: float = 0.01, decay_steps: int = 20000) -> float:
    ratio = min(float(step) / float(max(decay_steps, 1)), 1.0)
    return init_std + ratio * (final_std - init_std)


def get_smoothing_alpha(step: int, init_alpha: float = 0.0, final_alpha: float = 0.7, warmup_steps: int = 20000) -> float:
    ratio = min(float(step) / float(max(warmup_steps, 1)), 1.0)
    return init_alpha + ratio * (final_alpha - init_alpha)
    ...
    while simulation_app.is_running():
        ...
        if args.disable_tss:
            ...
            if (cached_z_t is None) or (env.common_step_counter % tss_update_interval == 0):
                ...
                # IMPORTANT:
                # cached tensors are reused across steps, so cut the graph here
                cached_z_t = cached_z_t.detach()
                cached_seq_idx = cached_seq_idx.detach()
                if cached_tss_scores is not None:
                    cached_tss_scores = cached_tss_scores.detach()
        ...
        current_std = get_action_std(env.common_step_counter)
        current_alpha = get_smoothing_alpha(env.common_step_counter)

        dist = torch.distributions.Normal(raw_actions, current_std)
        sampled_actions = dist.rsample()

        if prev_actions is None:
            actions = sampled_actions
        else:
            actions = current_alpha * prev_actions + (1.0 - current_alpha) * sampled_actions
        ...
        r_v = reward_vec[:, 0]
        r_s = reward_vec[:, 1]
        r_e = reward_vec[:, 2] # NOTE: r_e is currently closer to an effort/actuation-cost proxy than exact mechanical energy.
        ...
        reward_adv = (total_reward - total_reward.mean()).detach()
        loss_pg = -(log_prob * reward_adv).mean()
        policy_entropy = dist.entropy().sum(dim=-1).mean()

        loss = loss_pg - 0.001 * policy_entropy - 0.01 * beta_entropy
        ...
        if env.common_step_counter % 100 == 0:
            ...
            writer.add_scalar("Policy/ActionStd", float(current_std), step)
            writer.add_scalar("Policy/SmoothingAlpha", float(current_alpha), step)
            writer.add_scalar("Policy/Entropy", policy_entropy.item(), step)
