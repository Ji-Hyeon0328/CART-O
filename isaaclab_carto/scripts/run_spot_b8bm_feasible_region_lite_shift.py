...
#
# B8-bm: feasible-region-lite trunk shift.
#
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B8-bm feasible-region-lite trunk shift")
...
def make_base_ref(x_hat, foot_pos, ref_mask, prev_base_ref, cfg, phase, phase_step, active_leg=None):
    ...
    if (
        ...
        blended_xy = (1.0 - float(args.fr_lite_blend)) * base_ref[:, 0:2] + float(args.fr_lite_blend) * safe_xy
...
def print_debug(step, phase, profile, phase_step, x_hat, base_ref, trial_out, ref_mask, contact_mask,
    ...
    print("\n" + "=" * 150)
    print(f"[B8-bm FEASIBLE-REGION-LITE-SHIFT] step={step}")
    print("=" * 150)
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B8-bm feasible-region-lite trunk shift")
    ...
    for step in range(total_steps):
        ...
        if args.enable_real_swing_commit_state and swing_enabled:
            _sleg = LEG_TO_ID[args.test_leg] # type: ignore
