# isaaclab_carto/scripts/run_spot_b8ae_actuator_authority_probe.py
#
# B8-ae: Actuator authority probe for full-body WBC-QP v1.
#
# Motivation:
#   B8-ad closed the quasi-static full-body WBC standing loop.
#   However, applied_torque is still often much larger than tau_cmd, which
#   means the Isaac implicit actuator layer may be dominating the motion.
#
# Goal:
#   Keep the B8-ad full-WBC structure, but log actuator authority:
#
#     tau_cmd from WBC
#     applied_torque from Isaac
#     residual = applied_torque - tau_cmd
#     selected swing leg q/qdot/foot motion
#
# This is not a new controller; it is an actuator-layer isolation probe.
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B8-ae actuator authority probe")
...
# Diagnostic pulse on top of WBC, applied only during lift/hold_lift.
# This is a diagnostic, not final controller.
parser.add_argument("--pulse_hy", type=float, default=0.0)
parser.add_argument("--pulse_kn", type=float, default=0.0)
...
def add_diagnostic_pulse(tau, phase):
    if phase not in ["lift", "hold_lift"]:
        return tau
    leg = LEG_TO_ID[args.test_leg]
    tau = tau.clone()
    tau[:, HY[leg]] += args.pulse_hy
    tau[:, KN[leg]] += args.pulse_kn
    return tau


def print_debug(step, phase, profile, x_hat, base_ref, out, stance_mask, f_mpc, foot_pos, foot_swing0, swing_target, tau_cmd, qpd, robot, swing_enabled):
    leg = LEG_TO_ID[args.test_leg]
    ...
    denom = tau_cmd.abs().clamp_min(1.0)
    ratio = applied.abs() / denom

    print("\n" + "=" * 140)
    print(f"[B8-ae ACTUATOR-AUTHORITY-PROBE] step={step}")
    print("=" * 140)
    print("phase:", phase, "test_leg:", args.test_leg, "profile:", profile, "swing_enabled:", bool(swing_enabled[0].detach().cpu()))
    print("pd_scale:", args.pd_scale, "tau_cmd_scale:", args.tau_cmd_scale, "pulse_hy:", args.pulse_hy, "pulse_kn:", args.pulse_kn)
...
def main():
    ...
    print("\n" + "=" * 140)
    print("[INFO] Starting B8-ae actuator authority probe")
    print("test_leg:", args.test_leg, "mass:", mass, "pd_scale:", args.pd_scale)
    print("wbc_cfg:", wbc_cfg)
    print("=" * 140)

    last_tau_cmd = torch.zeros_like(robot.data.joint_pos)
    ...
    for step in range(total_steps):
        ...
        tau_cmd = add_diagnostic_pulse(tau_cmd, phase)
        last_tau_cmd = tau_cmd.detach().clone()

        env.step(tau_cmd)

        if step % max(args.print_every, 1) == 0:
            print_debug(
                step, phase, profile, x_hat, base_ref, out, stance_mask, f_mpc,
                foot_pos, foot_swing0, swing_target, last_tau_cmd, qpd, robot, swing_enabled
            )
