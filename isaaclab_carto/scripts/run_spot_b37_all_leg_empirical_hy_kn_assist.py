...
#
# B37: All-leg pure additive empirical HY+KN assist and movement check.
#
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B37 all-leg empirical HY+KN assist")
...
def print_debug(step, phase, profile, phase_step, x_hat, base_ref, trial_out, ref_mask, contact_mask,
    ...
    print("\n" + "=" * 150)
    print(f"[B37 ALL-LEG-EMPIRICAL-HY-KN-ASSIST] step={step}")
    print("=" * 150)
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B37 all-leg pure additive empirical HY+KN assist and movement check")
    ...
    for step in range(total_steps):
        ...
        if (args.b12_align_print or args.b16_recenter_align_print) and b12_align_info.get("active", False) and (step % max(args.print_every, 1) == 0):
            align_label = "[B25 RECENTER PD ALIGN]" if (isinstance(canonical_shift_info, dict) and canonical_shift_info.get("b15_recenter", False)) else "[B37 PD TARGET ALIGN]"
            print(align_label,
                  "step=", step,
