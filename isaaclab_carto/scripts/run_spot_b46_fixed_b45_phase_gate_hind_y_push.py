...
def _b45_clip_direction_with_latch_guard(q_now_axis, q_latch_axis, step_delta, max_abs_from_latch):
    ...
    Unlike B44, this function never reverses direction to chase q_latch/q_goal.
    If the requested step would exceed the guard, it either clips to the guard
    boundary or becomes zero when already outside and moving farther outward.
    """
    ...
    if step_f != 0.0 and clipped * step_f < 0.0:
        return 0.0, True
    return clipped, True
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B46 fixed B45 phase-gate hind y push bridge with B41 clamp")
