...
#
# B8-at: contact-ready forced release + RF single-step probe.
#
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B8-at contact-ready forced release + RF single-step probe")
...
def print_debug(step, phase, profile, phase_step, x_hat, base_ref, trial_out, ref_mask, contact_mask,
                ...
                old_forced, controlled_forced, ready_forced, calib_status, contact_ready_seen):
    leg = LEG_TO_ID[args.test_leg]
    ...
    print("\n" + "=" * 150)
    print(f"[B8-at CONTACT-READY-FORCED-RELEASE] step={step}")
    print("=" * 150)
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B8-at contact-ready forced release + RF single-step probe")
    ...
    for step in range(total_steps):
        ...
        ready_forced = contact_ready_release_gate(phase, phase_step, x_hat, contact_ready_seen)
        if ready_forced:
            swing_enabled = True
        if phase == "shift":
