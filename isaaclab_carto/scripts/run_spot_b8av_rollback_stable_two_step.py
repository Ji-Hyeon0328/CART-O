...
#
# B8-av: rollback to B8-aq stable upright baseline + optional conservative two-step sequence.
#
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B8-av rollback stable upright baseline + optional conservative two-step sequence")
...
def print_debug(step, phase, profile, phase_step, x_hat, base_ref, trial_out, ref_mask, contact_mask,
                ...
                old_forced, controlled_forced, step_idx):
    leg = LEG_TO_ID[args.test_leg]
    ...
    print("\n" + "=" * 150)
    print(f"[B8-av ROLLBACK-STABLE-TWO-STEP] step={step}")
    print("=" * 150)
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B8-av rollback stable upright baseline + optional conservative two-step sequence")
    ...
    for step in range(total_steps):
        ...
        ref_mask = all_stance_mask(args.num_envs, device, dtype) if phase in ["warmup", "settle", "done"] else future_swing_mask(args.num_envs, device, dtype)
        ...
        if phase in ["shift", "settle", "warmup", "done"]:
            # pre-shift / settle / warmup keep all contacts
            swing_enabled = False
