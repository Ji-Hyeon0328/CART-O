...
# B98 VERTICAL LIFT EMPIRICAL HY/KN OVERRIDES
...
args.b93_hold_after_release_steps = 14
...
args.b98_max_hy_delta = 0.012
args.b98_max_kn_delta = 0.012
args.b98_print = True
print('[B98 CONFIG] vertical-only LH/RH micro-lift + empirical HY/KN z-error assist')
...
def _b70_minimal_clean_swing_adapter(robot, Jfeet_full, foot_pos_arg, b23_info, step):
    ...
    try:
        ...
        if bool(getattr(args, "enable_b74_hykn_soft_hold", True)):
        ...
        if bool(getattr(args, "enable_b98_vertical_lift_empirical_hykn", False)):
            try:
                is_forced_vertical = (str(b23_info.get("xy_mode", "")) == "b95_forced") or ("b95" in str(b23_info.get("b95_reason", "")))
            except Exception:
        ...
        if bool(getattr(args, "enable_b75_lateral_hx_guard", True)):
        ...
        if bool(getattr(args, "b70_apply_torque_assist", False)) or (bool(getattr(args, "enable_b76_walk_authority", True)) and bool(getattr(args, "b76_enable_small_torque", True))):
            qd = robot.data.joint_vel.detach()
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B98 VERTICAL LIFT EMPIRICAL HY/KN + B97/B96 recovery planner")
    ...
    for step in range(total_steps):
        ...
        try:
            ...
            if _b95_active:
                ...
                if bool(getattr(args, "b95_print", True)) and (step == b95_pulse_start_step or step % max(args.print_every, 1) == 0):
                    print("[B98/B95 FORCED VERTICAL MICRO-LIFT FOOTREF]",
                          "step=", step,
                ...
                if bool(getattr(args, "enable_b95_forced_hind_pulse_footref", False)) and b95_pulse_leg is not None:
                    if bool(getattr(args, "b95_print", True)) and step % max(args.print_every, 1) == 0:
                        print("[B98/B95 FORCED VERTICAL MICRO-LIFT FOOTREF END]", "step=", step, "leg=", ID_TO_LEG.get(int(b95_pulse_leg), b95_pulse_leg))
                b95_pulse_leg = None
                b95_pulse_anchor_w = None
                b95_pulse_target_w = None
                b95_pulse_start_step = -1
            ...
            if bool(getattr(args, "b95_print", True)) and step % max(args.print_every, 1) == 0:
                print("[B98/B95 FORCED VERTICAL MICRO-LIFT FOOTREF ERROR]", "step=", step, "error=", repr(_b95_e))
        ...
        try:
            ...
            if bool(getattr(args, "b91_print", True)) and isinstance(b91_force_bias_info, dict) and (b91_force_bias_info.get("active", False) or step % max(args.print_every, 1) == 0):
