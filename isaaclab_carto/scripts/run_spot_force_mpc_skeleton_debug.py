# isaaclab_carto/scripts/run_spot_force_mpc_skeleton_debug.py
#
# B5-a/B5-b: forceMPC skeleton debug.
#
# This script runs:
#   thetaDecoder / thetaRefMapper
#   → Ref.S
#   → beta_t
#   → unconstrained centroidal LS force planner
#   → tau = J^T f
#
# It is NOT full constrained forceMPC yet.
...
parser = argparse.ArgumentParser(description="CARTO Spot forceMPC skeleton debug")
...
from isaaclab_carto.lowlevel.force_mpc_skeleton import ForceMPCSkeletonState, make_force_mpc_skeleton_torque  # noqa: E402
...
def print_debug(step, robot, x_hat_before, u_cmd, beta_t, theta, ref, tau_cmd, info):
    ...
    print("\n" + "-" * 120)
    print(f"[FORCE MPC SKELETON DEBUG] step={step}")
    print("-" * 120)
    ...
    keys = [
        ...
        "a_des_env0", "f_feet_env0", "f_norm_env0",
...
def main():
    ...
    planner_state = ForceMPCSkeletonState()

    print("\n" + "=" * 120)
    print("[INFO] Starting Spot forceMPC skeleton debug")
    print("=" * 120)
    print(f"z_mode={args.z_mode}, beta_preset={args.beta_preset}, u_cmd=[{args.vx},{args.vy},{args.wz}]")
    print(f"residual_ratio={args.residual_ratio}, max_tau={args.max_tau}")
    print("=" * 120 + "\n")
    ...
    for step in range(args.steps):
        ...
        tau_cmd, info = make_force_mpc_skeleton_torque(
            robot=robot,
            ref=ref,
            x_hat=x_hat,
            u_cmd=u_cmd,
            ...
            max_fxy_per_foot=args.max_fxy_per_foot,
        ...
        if step % args.print_every == 0:
            print_debug(step, robot, x_hat, u_cmd, beta_t, theta, ref, tau_cmd, info)
