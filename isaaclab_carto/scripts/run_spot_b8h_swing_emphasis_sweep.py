# isaaclab_carto/scripts/run_spot_b8h_swing_emphasis_sweep.py
#
# B8-h: Swing-emphasis sweep under balance-augmented QPS-WBC.
#
# Based on B8-g:
#   - balance augmentation stabilized pd_scale=0.50
#   - but visible swing motion remained weak
#
# Goal:
#   Keep the B8-g Run 2 balance setting, then gradually increase swing task
#   authority while monitoring:
#
#       base z drop
#       pitch drift
#       settled foot_delta
#       swing-leg foot_delta
#       applied / command torque ratio
#
# This is still not "walking" yet.
# It is a controlled sweep to find the boundary:
#
#   stable standing + measurable swing motion
#
# After this, we can move to B8-i:
#   visible stepping / in-place gait with tuned balance + swing weights.
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B8-h swing emphasis sweep")
...
# balance augmentation - defaults copied from B8-g Run 2
...
parser.add_argument("--w_swing", type=float, default=0.25)
parser.add_argument("--w_posture", type=float, default=0.45)
parser.add_argument("--w_reg", type=float, default=0.03)
parser.add_argument("--w_rate", type=float, default=0.25)

parser.add_argument("--kp_swing_xy", type=float, default=18.0)
parser.add_argument("--kp_swing_z", type=float, default=30.0)
parser.add_argument("--kd_swing_xy", type=float, default=1.5)
parser.add_argument("--kd_swing_z", type=float, default=2.2)
parser.add_argument("--max_task_cmd", type=float, default=3.5)
parser.add_argument("--max_pos_err", type=float, default=0.045)
...
def compute_swing_leg_delta(foot_delta, swing_mask):
    # foot_delta: [N,4,3], swing_mask: list length 4 from info
...
def print_probe_debug(
    ...
    swing_mask = wbc_info.get("swing_mask_env0", [0.0, 0.0, 0.0, 0.0])
    swing_delta_max, swing_delta_mean = compute_swing_leg_delta(foot_delta_settle, swing_mask)

    print("\n" + "=" * 132)
    print(f"[B8-h SWING EMPHASIS SWEEP] probe_step={probe_step}")
    print("=" * 132)
...
def main():
    ...
    print("\n" + "=" * 132)
    print("[INFO] Starting B8-h swing emphasis sweep")
    print("=" * 132)
    print(f"pd_scale={args.pd_scale}, settle_steps={args.settle_steps}, probe_steps={args.probe_steps}, settle_mode={args.settle_mode}")
    print(f"w_swing={args.w_swing}, kp_swing_z={args.kp_swing_z}, max_task_cmd={args.max_task_cmd}, tau_probe_scale={args.tau_probe_scale}")
    print("=" * 132 + "\n")
