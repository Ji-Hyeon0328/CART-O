    ...
    try:
        ...
        if bool(getattr(args, "enable_b75_lateral_hx_guard", True)):
            ...
            if torch.abs(e_y) > dead:
                ...
                hx_delta = torch.clamp(hx_delta, -float(getattr(args, "b75_max_hx_delta", 0.014)), float(getattr(args, "b75_max_hx_delta", 0.014)))
                dq[0] = hx_delta
        ...
        step_cap = torch.tensor([[float(getattr(args, "b70_max_hx_delta", 0.010)),
        ...
        if bool(getattr(args, "b70_apply_torque_assist", False)):
            qd = robot.data.joint_vel.detach()
            tau_leg = float(getattr(args, "b70_torque_kp", 14.0)) * (q_cmd - q_now[:, joint_ids]) - float(getattr(args, "b70_torque_kd", 1.5)) * qd[:, joint_ids]
            tau_leg = torch.clamp(tau_leg, -float(getattr(args, "b70_max_tau", 3.0)), float(getattr(args, "b70_max_tau", 3.0)))
            tau[:, joint_ids] = tau_leg
        else:
            tau_leg = torch.zeros((q_now.shape[0], 3), device=q_now.device, dtype=q_now.dtype)
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B75 lateral-HX guard + B74 HY/KN soft-hold clean swing adapter + B69/B68 isolation planner")
