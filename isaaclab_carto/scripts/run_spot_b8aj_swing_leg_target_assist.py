# isaaclab_carto/scripts/run_spot_b8aj_swing_leg_target_assist.py
#
# B8-aj: No-align decoupled pre-shift + swing-leg-only target assist for full-WBC.
#
...
parser.add_argument("--swing_tau_assist", action="store_true")
parser.add_argument("--swing_tau_hx", type=float, default=0.0)
parser.add_argument("--swing_tau_hy", type=float, default=0.0)
parser.add_argument("--swing_tau_kn", type=float, default=0.0)
parser.add_argument("--assist_start_profile", type=float, default=0.15)
parser.add_argument("--assist_max_abs", type=float, default=6.0)

# Swing-leg-only implicit target assist.
# Unlike global --align_targets, this is activated only after swing_enabled=True.
# The full target tensor is initialized from q_nom, but only the selected swing leg is offset.
parser.add_argument("--swing_target_assist_only", action="store_true")
parser.add_argument("--swing_target_base", type=str, default="nominal", choices=["nominal", "current"])
parser.add_argument("--swing_target_hx_offset", type=float, default=0.0)
parser.add_argument("--swing_target_hy_offset", type=float, default=0.0)
parser.add_argument("--swing_target_kn_offset", type=float, default=0.0)
parser.add_argument("--swing_target_start_profile", type=float, default=0.20)
parser.add_argument("--swing_target_max_delta", type=float, default=0.10)
...
def apply_swing_leg_target_assist(robot, q_nom, phase, profile, swing_enabled):
    """Apply an implicit target only for a selected swing-leg test.

    This deliberately avoids the previous global align_targets behavior:
      - it is inactive during warmup and pre-shift,
      - it activates only after contact_mask has switched and swing_enabled=True,
      - it uses very small smooth offsets on the selected leg,
      - it is intended as a motor-interface diagnostic, not final WBC logic.
    """
    if not args.swing_target_assist_only:
        return None
    if not swing_enabled:
        return None
    if phase not in ["lift", "hold_lift", "lower"]:
        return None
    if profile < args.swing_target_start_profile:
        return None

    q = robot.data.joint_pos.detach()
    if args.swing_target_base == "current":
        q_target = q.clone()
    else:
        q_target = q_nom.clone()

    leg = LEG_TO_ID[args.test_leg]
    denom = max(1e-6, 1.0 - args.swing_target_start_profile)
    s = float(max(0.0, min(1.0, (profile - args.swing_target_start_profile) / denom)))

    offsets = {
        HX[leg]: args.swing_target_hx_offset,
        HY[leg]: args.swing_target_hy_offset,
        KN[leg]: args.swing_target_kn_offset,
    }
    for jid, off in offsets.items():
        off = max(-args.swing_target_max_delta, min(args.swing_target_max_delta, float(off)))
        q_target[:, jid] = q_target[:, jid] + off * s

    robot.set_joint_position_target(q_target)
    robot.set_joint_velocity_target(torch.zeros_like(q_target))
    return q_target



def apply_swing_torque_assist(tau_cmd, phase, profile, swing_enabled):
    """Add local torque bias only to the selected swing leg.

    No set_joint_position_target call.
    No stance-leg target change.
    Assist is active only after swing_enabled=True.
    """
    assist = torch.zeros_like(tau_cmd)
    if not args.swing_tau_assist:
        return tau_cmd, assist
    if not swing_enabled:
        return tau_cmd, assist
    if phase not in ["lift", "hold_lift", "lower"]:
        return tau_cmd, assist
    if profile < args.assist_start_profile:
        return tau_cmd, assist

    leg = LEG_TO_ID[args.test_leg]
    denom = max(1e-6, 1.0 - args.assist_start_profile)
    s = float(max(0.0, min(1.0, (profile - args.assist_start_profile) / denom)))
    for jid, val in [(HX[leg], args.swing_tau_hx), (HY[leg], args.swing_tau_hy), (KN[leg], args.swing_tau_kn)]:
        val = max(-args.assist_max_abs, min(args.assist_max_abs, float(val)))
        assist[:, jid] = val * s

    tau_out = torch.clamp(tau_cmd + assist, -args.max_tau, args.max_tau)
    return tau_out, assist


def print_debug(step, phase, profile, phase_step, x_hat, base_ref, trial_out, ref_mask, contact_mask,
                f_mpc, foot_pos, foot_swing0, swing_target, tau_cmd, qpd, robot, swing_enabled, q_target, tau_assist):
    leg = LEG_TO_ID[args.test_leg]
    ...
    print("\n" + "=" * 150)
    print(f"[B8-aj SWING-LEG-TARGET-ASSIST] step={step}")
    print("=" * 150)
    ...
    print("tau_assist:", tau_assist[0, idx].detach().cpu().numpy())
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B8-aj swing-leg-only target assist")
    ...
    for step in range(total_steps):
        ...
        tau_cmd, tau_assist = apply_swing_torque_assist(tau_cmd, phase, profile, swing_enabled)
        q_target = None
        if args.align_targets:
            q_target = apply_implicit_target_alignment(robot, q_nom, phase, profile, swing_enabled)
        else:
            q_target = apply_swing_leg_target_assist(robot, q_nom, phase, profile, swing_enabled)
