# isaaclab_carto/scripts/run_spot_b8z_wbc_qp_lite.py
#
# B8-z: design-aligned WBC-QP-lite test.
#
# This is the next step after B8-y:
#   - keep contact schedule S
#   - keep support-region base reference
#   - keep MPC-like GRF f_mpc
#   - replace simple task composition with a least-squares WBC-QP-lite
#
# Decision variables:
...
#   tau -> posture tau / regularized
#
# This is still not final floating-base WBC, but it is a firmer
# implementation and follows the intended research design more closely.
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B8-z WBC-QP-lite")
...
parser.add_argument("--w_stance_acc", type=float, default=8.0)
parser.add_argument("--w_swing_acc", type=float, default=12.0)
parser.add_argument("--w_tau_posture", type=float, default=0.20)
...
parser.add_argument("--max_swing_acc", type=float, default=8.0)
...
from isaaclab_carto.lowlevel.tracer_wbc_qp_lite import WbcQpLiteConfig, solve_wbc_qp_lite  # noqa: E402
...
def get_mass_matrix(robot):
    # Best-effort; if unavailable, QP-lite falls back to identity.
    try:
...
def print_debug(step, phase, profile, x_hat, base_ref, out, stance_mask, f_mpc, wrench, foot_pos, foot0, foot_swing0, swing_target, tau, qpd, robot, swing_enabled):
    ...
    print("\n" + "=" * 132)
    print(f"[B8-z WBC-QP-LITE] step={step}")
    print("=" * 132)
    ...
    print("qdd_pred max_abs:", float(qpd["qdd_pred"].abs().max().detach().cpu()))
...
def main():
    ...
    print("\n" + "=" * 132)
    print("[INFO] Starting B8-z WBC-QP-lite")
