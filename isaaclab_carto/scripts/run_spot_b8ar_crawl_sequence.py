...
#
# B8-ar: multi-leg crawl sequence with upright forced release + Jacobian-IK swing adapter.
#
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B8-ar crawl sequence + Jacobian-IK swing adapter")
...
parser.add_argument("--gait_order", type=str, default="RF,LH,LF,RH",
                    help="Comma-separated leg order, e.g. RF,LH,LF,RH")
parser.add_argument("--gait_cycles", type=int, default=1)
...
def parse_gait_order():
    order = [x.strip().upper() for x in args.gait_order.split(",") if x.strip()]
    if not order:
        order = [args.test_leg]
    for leg in order:
        if leg not in LEG_TO_ID:
            raise ValueError(f"Invalid leg in --gait_order: {leg}")
    return order
...
def get_phase(step):
    """Return phase/profile/phase_step/active_leg/cycle_index.

    Warmup is shared. After warmup, each leg in gait_order performs:
      shift -> lift -> hold_lift -> lower -> settle
    """
    order = parse_gait_order()

    if step < args.warmup_steps:
        return "warmup", 0.0, step, order[0], -1

    t_global = step - args.warmup_steps
    per = one_step_len()
    seq_idx = int(t_global // max(per, 1))
    t = int(t_global % max(per, 1))

    leg = order[seq_idx % len(order)]
    cycle_idx = seq_idx // len(order)

    s, l, h, lo, st = phase_lengths()

    if t < s:
        return "shift", 0.0, t, leg, cycle_idx
    t -= s
    if t < l:
        return "lift", smooth01(t / max(l, 1)), t, leg, cycle_idx
    t -= l
    if t < h:
        return "hold_lift", smooth01(t / max(h, 1)), t, leg, cycle_idx
    t -= h
    if t < lo:
        return "lower", smooth01(1.0 - t / max(lo, 1)), t, leg, cycle_idx
    t -= lo
    if t < st:
        return "settle", 0.0, t, leg, cycle_idx
    return "done", 0.0, t, leg, cycle_idx
...
def print_debug(step, phase, profile, phase_step, x_hat, base_ref, trial_out, ref_mask, contact_mask,
                ...
                old_forced, controlled_forced, cycle_idx):
    leg = LEG_TO_ID[args.test_leg]
    ...
    print("\n" + "=" * 150)
    print(f"[B8-ar CRAWL-SEQUENCE] step={step}")
    print("=" * 150)
    print("phase:", phase, "phase_step:", phase_step, "test_leg:", args.test_leg, "cycle_idx:", cycle_idx, "profile:", profile)
...
def main():
    ...
    total_steps = args.warmup_steps + one_step_len() * len(parse_gait_order()) * args.gait_cycles

    print("\n" + "=" * 150)
    print("[INFO] Starting B8-ar crawl sequence + Jacobian-IK swing adapter")
    ...
    print("crawl_sequence:",
          "gait_order:", parse_gait_order(),
          "gait_cycles:", args.gait_cycles,
    ...
    for step in range(total_steps):
        ...
        phase, profile, phase_step, active_leg, cycle_idx = get_phase(step)
        # Most helper functions still read args.test_leg. Update it per sequence step.
        if args.test_leg != active_leg:
            args.test_leg = active_leg

        step_key = (cycle_idx, active_leg)
