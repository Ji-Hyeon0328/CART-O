...
args.b110_only_during_forced_vertical_pulse = True
...
def _b70_minimal_clean_swing_adapter(robot, Jfeet_full, foot_pos_arg, b23_info, step):
    ...
    try:
        ...
        if b110_active and bool(getattr(args, "enable_b112_ff_torque_sign_sweep", False)):
            start = int(locals().get("b95_pulse_start_step", -1))
            if start >= 0:
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B112 CLEAN FF TORQUE SIGN SWEEP (NO RECOVERY, NO PD, Q-TARGET OFF) planner")
