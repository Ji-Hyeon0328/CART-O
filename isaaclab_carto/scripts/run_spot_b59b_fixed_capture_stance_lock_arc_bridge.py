...
def _b55_capture_touchdown_ik_bridge(robot, Jfeet_full, foot_pos_arg, b23_info, step):
    ...
    try:
        ...
        # touchdown height with meaningful forward progress, stop commanding extra
        ...
        if bool(getattr(args, "enable_b59_capture_stance_lock", False)):
            ...
            if phase in ("capture", "post_capture_extra") and (not b59_allowed or leg_name in b59_allowed):
                ...
                min_forward = float(getattr(args, "b59_lock_min_forward_from_anchor", 0.035))
                ...
                should_lock = (z_above_target <= z_tol and forward_from_anchor >= min_forward and y_err_abs <= max_y_err)
            ...
            if phase in ("capture", "post_capture_extra") and (not b59_allowed or leg_name in b59_allowed):
                ...
                if leg_name in locks or should_lock:
                    ...
                    info.update({
                        ...
                        "clamped": torch.zeros(3, dtype=torch.bool).cpu().numpy(), "tau_leg": tau_leg[0].detach().cpu().numpy(),
                        "max_delta": 0.0, "max_tau": 0.0,
                        "b59_forward_from_anchor": forward_from_anchor, "b59_z_above_target": z_above_target, "b59_y_err_abs": y_err_abs,
                    })
                    return q_target, tau, info
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B59b FIXED capture-stance lock inside B57 + B58/B57/B56b/B49/B48 planner")
