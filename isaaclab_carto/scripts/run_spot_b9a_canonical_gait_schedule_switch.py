...
#
# B9-a: canonical gait schedule switch.
#
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B9-a canonical gait schedule switch")
...
def compute_canonical_gait_schedule(step, num_envs, device, dtype):
    ...
    phase_scalar = (float(step) / float(T)) % 1.0
...
def print_debug(step, phase, profile, phase_step, x_hat, base_ref, trial_out, ref_mask, contact_mask,
    ...
    print("\n" + "=" * 150)
    print(f"[B9-a CANONICAL-GAIT-SCHEDULE-SWITCH] step={step}")
    print("=" * 150)
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B9-a canonical gait schedule switch")
