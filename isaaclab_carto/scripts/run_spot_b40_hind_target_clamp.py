...
def _b18_commit_or_reuse_foothold_target(canonical_foot_target_w, b17_info, canonical_s_t, canonical_sigma_t,
    ...
    if (not bool(committed_valid[leg])) and isinstance(b17_info, dict) and b17_info.get("active", False) and sigma >= float(args.b18_commit_min_sigma):
        ...
        # B40 safety filter: prevent hind-leg committed target from jumping too far from the swing anchor.
        if bool(getattr(args, "enable_b40_hind_target_clamp", False)) and canonical_foot_target_w is not None:
            try:
                anchor_for_clamp = canonical_foot_target_w[0, leg, :].detach().clone()
                if 'canonical_foot_anchor_w' in globals() and canonical_foot_anchor_w is not None:
                    pass
                # Use the current canonical target as a fallback anchor here; B23 will clamp again with the true canonical anchor.
...
def _b23_apply_wbc_main_committed_footref(canonical_foot_target_w, canonical_foot_anchor_w,
    ...
    if bool(getattr(args, "enable_b40_hind_target_clamp", False)) and canonical_foot_anchor_w is not None:
        try:
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B39 latched candidate-specific support-region recenter before hind-leg liftoff")
