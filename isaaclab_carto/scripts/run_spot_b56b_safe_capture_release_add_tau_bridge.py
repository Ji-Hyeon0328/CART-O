...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B56b safe capture release bridge with B56 torque integration + B54/B49/B48 planner")
    ...
    for step in range(total_steps):
        ...
        if 'b53_tau_assist' in locals() and isinstance(b53_visible_info, dict) and b53_visible_info.get("active", False):
            ...
            tau_cmd = torch.clamp(tau_cmd, -float(args.max_tau), float(args.max_tau))
        if 'b54_tau_assist' in locals() and isinstance(b54_visible_info, dict) and b54_visible_info.get("active", False):
        ...
        if 'b56_tau_assist' in locals() and isinstance(b56_safe_info, dict) and b56_safe_info.get("active", False):
