# isaaclab_carto/scripts/run_spot_projected_force_mpc_debug.py
#
# B5-c: projected forceMPC debug.
#
# This is a QP-like projected planner:
#   LS force solve
#   + stance/swing mask
#   + fz bounds
#   + friction pyramid
...
parser = argparse.ArgumentParser(description="CARTO Spot projected forceMPC debug")
...
from isaaclab_carto.lowlevel.projected_force_mpc import ProjectedForceMPCState, make_projected_force_mpc_torque  # noqa: E402
...
def print_debug(step, robot, u_cmd, beta_t, theta, ref, tau_cmd, info):
    ...
    print("\n" + "-" * 130)
    print(f"[PROJECTED FORCE MPC DEBUG] step={step}")
    print("-" * 130)
    ...
    keys = [
        ...
        "v_des_xy_env0", "v_world_xy_env0", "v_err_xy_env0",
        "a_des_env0",
        "f_raw_env0", "f_projected_env0", "f_norm_env0",
        "mu", "fz_min", "fz_max", "fxy_abs_max",
        "projection_pass1", "projection_pass2", "projection_pass3",
        "force_rate_clamp_count", "max_force_delta_before", "max_delta_f_env0",
...
def main():
    ...
    planner_state = ProjectedForceMPCState()

    print("\n" + "=" * 130)
    print("[INFO] Starting Spot projected forceMPC debug")
    print("=" * 130)
    print(f"z_mode={args.z_mode}, beta_preset={args.beta_preset}, u_cmd=[{args.vx},{args.vy},{args.wz}]")
    print(f"residual_ratio={args.residual_ratio}, mu={args.mu}, fz=[{args.fz_min},{args.fz_max}], max_delta_f={args.max_delta_f}")
    print("=" * 130 + "\n")
    ...
    for step in range(args.steps):
        ...
        tau_cmd, info = make_projected_force_mpc_torque(
            robot=robot,
            ref=ref,
            x_hat=x_hat,
            u_cmd=u_cmd,
