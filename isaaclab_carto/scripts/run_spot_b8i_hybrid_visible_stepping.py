# isaaclab_carto/scripts/run_spot_b8i_hybrid_visible_stepping.py
#
# B8-i: Hybrid visible stepping debug.
#
# Motivation:
#   B8-g/B8-h stabilized the robot under reduced implicit PD + balance WBC,
#   but swing motion remained visually tiny.
#
# Idea:
#   Keep the balance-augmented QPS-WBC residual for standing support,
#   then add an explicit joint-space swing-lift residual only on scheduled
#   swing legs.
#
#   tau_cmd =
#       tau_probe_scale * tau_balance_wbc
#       + swing_joint_scale * tau_swing_joint
#
# where tau_swing_joint is:
#   Kp (q_swing_des - q) - Kd dq
#
# Important:
#   This is a hybrid visibility bridge, not final WBC.
#   The goal is to verify that Ref.S can drive visible stepping while
#   balance augmentation prevents the sit-down behavior observed in B8-f.
#
# If it sits/falls:
#   reduce swing_joint_scale, kp_swing_joint, offsets, or increase pd_scale.
#
# If there is still no visible motion:
#   increase swing_joint_scale or offsets gradually.
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B8-i hybrid visible stepping debug")

parser.add_argument("--num_envs", type=int, default=1)
parser.add_argument("--settle_steps", type=int, default=120)
parser.add_argument("--probe_steps", type=int, default=240)
...
# balance augmentation - based on stable B8-g Run 2
...
parser.add_argument("--w_swing", type=float, default=0.15)
parser.add_argument("--w_posture", type=float, default=0.50)
parser.add_argument("--w_reg", type=float, default=0.04)
parser.add_argument("--w_rate", type=float, default=0.30)

parser.add_argument("--kp_swing_xy", type=float, default=10.0)
parser.add_argument("--kp_swing_z", type=float, default=18.0)
parser.add_argument("--kd_swing_xy", type=float, default=1.0)
parser.add_argument("--kd_swing_z", type=float, default=1.5)
parser.add_argument("--max_task_cmd", type=float, default=2.0)
parser.add_argument("--max_pos_err", type=float, default=0.03)

parser.add_argument("--kp_posture", type=float, default=1.2)
parser.add_argument("--kd_posture", type=float, default=0.06)
parser.add_argument("--max_posture_tau", type=float, default=0.30)
...
parser.add_argument("--swing_joint_scale", type=float, default=1.0)
parser.add_argument("--kp_swing_joint", type=float, default=18.0)
parser.add_argument("--kd_swing_joint", type=float, default=0.8)
parser.add_argument("--max_swing_joint_tau", type=float, default=3.0)
parser.add_argument("--hy_lift_delta", type=float, default=-0.06)
parser.add_argument("--kn_lift_delta", type=float, default=0.14)
parser.add_argument("--hy_sweep_delta", type=float, default=0.010)
...
def make_hybrid_swing_joint_torque(robot, ref, theta, q_swing_base, k: int):
    ...
    info = {
        ...
        "q_swing_base_env0": q_swing_base[0].detach().cpu().tolist(),
        "q_des_swing_env0": q_des[0].detach().cpu().tolist(),
        "q_err_swing_env0": q_err[0].detach().cpu().tolist(),
        "tau_swing_joint_env0": tau[0].detach().cpu().tolist(),
        ...
        "kd_swing_joint": args.kd_swing_joint,
...
def print_probe_debug(
    ...
    print("\n" + "=" * 132)
    print(f"[B8-i HYBRID VISIBLE STEPPING] probe_step={probe_step}")
    print("=" * 132)
    ...
    for key in [
        ...
        "Fz_balance_env0", "My_balance_env0",
        "fz_bias_env0", "f_bias_norm_env0", "f_aug_norm_env0",
    ]:
        if key in wbc_info:
    ...
    for key in [
        ...
        "hy_offset_env0",
        "kn_offset_env0",
        "tau_swing_joint_mean_abs",
        "tau_swing_joint_max_abs",
        "swing_joint_scale",
        "kp_swing_joint",
        "max_swing_joint_tau",
    ]:
    ...
    print("tau_cmd env0:", tau_cmd[env_id].detach().cpu().numpy())
...
def main():
    ...
    print("\n" + "=" * 132)
    print("[INFO] Starting B8-i hybrid visible stepping")
    print("=" * 132)
    print(f"pd_scale={args.pd_scale}, enable_hybrid_swing={args.enable_hybrid_swing}")
    print(f"swing_joint_scale={args.swing_joint_scale}, kp_swing_joint={args.kp_swing_joint}, max_swing_joint_tau={args.max_swing_joint_tau}")
    print("=" * 132 + "\n")
