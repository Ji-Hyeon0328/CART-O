...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B86 NO-FUTURE-SHIFT BEFORE HIND + B85/B83/B82 recovery planner")
    ...
    print("b86_no_future_shift_before_hind:",
    ...
    for step in range(total_steps):
        ...
        # B86: do not pull the trunk toward a future support region before a protected
        # hind leg is actually released.  This specifically tests the hypothesis that
        # the pre-LH B39/B11/B12 shift injects the lateral/forward momentum seen in B85.
        ...
        try:
            if bool(getattr(args, "enable_b86_no_future_shift_before_hind", True)) and isinstance(canonical_shift_info, dict):
                ...
                _is_pre_liftoff = not bool(swing_enabled)
                _not_glide = int(b85_settle_glide_count) <= 0
                if _cand_name is not None and _cand_name.upper() in _protected and _is_pre_liftoff and _not_glide:
                    ...
                    if bool(getattr(args, "b86_print", True)) and (step % max(args.print_every, 1) == 0):
                        print("[B86 NO-FUTURE-SHIFT BEFORE HIND]",
                              "step=", step,
                              "candidate=", _cand_name,
                              "base_ref_xy=", base_ref[0,0:2].detach().cpu().numpy(),
                              "base_xy=", x_hat[0,0:2].detach().cpu().numpy(),
                              "reason=", "pre_liftoff_current_body_ref")
        except Exception as _b86_e:
            if bool(getattr(args, "b86_print", True)) and (step % max(args.print_every, 1) == 0):
                print("[B86 NO-FUTURE-SHIFT ERROR]", "step=", step, "error=", repr(_b86_e))
        ...
        try:
            if bool(locals().get("b86_pre_hind_shift_disabled", False)):
                ...
                b11_reloc_info = {"active": False, "reason": "b86_disabled_pre_hind_shift", "candidate": canonical_shift_info.get("candidate", None) if isinstance(canonical_shift_info, dict) else None}
                b12_align_info = {"active": False, "reason": "b86_disabled_pre_hind_shift"}
                q_nom_active = q_nom
        except Exception:
            pass
