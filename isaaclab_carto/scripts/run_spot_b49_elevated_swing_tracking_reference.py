...
def _b49_make_elevated_tracking_targets(committed_targets, committed_valid,
    ...
    try:
        ...
        if canonical_foot_anchor_w is not None:
            ...
            z_min = z_anchor + float(getattr(args, "b49_min_clearance", 0.035)) * math.sin(math.pi * sigma)
            z_min_t = torch.as_tensor(z_min, device=z_ref.device, dtype=z_ref.dtype)
            z_new = torch.maximum(z_ref + float(getattr(args, "b49_z_boost", 0.0)), z_min_t)
        else:
            z_min_t = z_ref
            z_new = z_ref + float(getattr(args, "b49_z_boost", 0.0))
        ...
        info.update({
            ...
            "new_target": out[leg, :].detach().cpu().numpy(),
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B49 elevated swing tracking reference + B48 post-landing feasibility planner")
