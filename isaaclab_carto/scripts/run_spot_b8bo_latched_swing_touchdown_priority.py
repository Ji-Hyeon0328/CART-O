...
#
# B8-bo: latched swing with touchdown-priority gate.
#
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B8-bo latched swing with touchdown-priority gate")
...
def print_debug(step, phase, profile, phase_step, x_hat, base_ref, trial_out, ref_mask, contact_mask,
    ...
    print("\n" + "=" * 150)
    print(f"[B8-bo LATCHED-SWING-TOUCHDOWN-PRIORITY] step={step}")
    print("=" * 150)
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B8-bo latched swing with touchdown-priority gate")
    ...
    for step in range(total_steps):
        ...
        freeze_base_now = (
            ...
            or (args.freeze_base_ref_during_touchdown_recovery and (touchdown_gate_active or freeze_step_key_for_touchdown or late_touchdown_hold_swing or (args.latched_swing_keep_base_freeze and swing_latched) or post_touchdown_lock_count > 0))
        )
        if freeze_base_now:
        ...
        if post_touchdown_lock_count > 0 and post_touchdown_lock_leg is not None:
            post_touchdown_lock_target, post_touchdown_lock_delta_full, post_touchdown_lock_info = apply_foothold_lock_recovery_target(
                robot, Jfeet_full, foot_pos, int(post_touchdown_lock_leg), post_touchdown_lock_foothold_target_w, True
            )
