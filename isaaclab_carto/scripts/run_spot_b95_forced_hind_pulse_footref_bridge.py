...
# B95 FORCED HIND PULSE FOOTREF OVERRIDES
# B94 proved that a short safe pulse can open LH without immediate collapse,
# but B23/B70 did not attach because no committed foothold existed during the
# pulse.  B95 latches a tiny committed foot target at pulse start and injects
# an active B23-style footref so B70 can actually command a hind micro-swing.
...
print('[B95 CONFIG] forced hind pulse footref: B94 safe pulse + latched tiny LH/RH target for B23/B70')
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B95 FORCED HIND PULSE FOOTREF + B94/B93/B92 recovery planner")
    ...
    for step in range(total_steps):
        ...
        try:
            ...
            if _b95_active:
                ...
                b23_footref_info = {
                    "active": True,
                    "reason": "b95_forced_pulse_active_swing",
                ...
                if bool(getattr(args, "b95_print", True)) and (step == b95_pulse_start_step or step % max(args.print_every, 1) == 0):
                    print("[B95 FORCED HIND PULSE FOOTREF]",
                          "step=", step,
                ...
                if bool(getattr(args, "enable_b95_forced_hind_pulse_footref", False)) and b95_pulse_leg is not None:
                    if bool(getattr(args, "b95_print", True)) and step % max(args.print_every, 1) == 0:
                        print("[B95 FORCED HIND PULSE FOOTREF END]", "step=", step, "leg=", ID_TO_LEG.get(int(b95_pulse_leg), b95_pulse_leg))
                b95_pulse_leg = None
                b95_pulse_anchor_w = None
                b95_pulse_target_w = None
                b95_pulse_start_step = -1
            ...
            if bool(getattr(args, "b95_print", True)) and step % max(args.print_every, 1) == 0:
                print("[B95 FORCED HIND PULSE FOOTREF ERROR]", "step=", step, "error=", repr(_b95_e))
