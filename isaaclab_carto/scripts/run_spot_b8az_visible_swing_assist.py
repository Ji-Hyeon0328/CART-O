...
#
# B8-az: visible swing assist probe.
#
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B8-az visible swing assist probe")
...
def visible_swing_assist_delta(leg, profile, q_now):
...
def apply_jacobian_ik_swing_target(robot, Jfeet_full, foot_pos, swing_target, phase, profile, swing_enabled):
    ...
    assist_delta_hxhykn = visible_swing_assist_delta(leg, profile, q_now)
    if args.ik_use_hx:
...
def print_debug(step, phase, profile, phase_step, x_hat, base_ref, trial_out, ref_mask, contact_mask,
    ...
    print("\n" + "=" * 150)
    print(f"[B8-az VISIBLE-SWING-ASSIST] step={step}")
    print("=" * 150)
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B8-az visible swing assist probe")
    ...
    for step in range(total_steps):
        ...
        if args.auto_select_second_leg and args.enable_two_step_sequence and step_idx == 1:
