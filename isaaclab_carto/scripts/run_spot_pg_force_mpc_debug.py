# isaaclab_carto/scripts/run_spot_pg_force_mpc_debug.py
#
# B5-d: projected-gradient QP-style forceMPC debug.
#
# This returns to the effort/residual environment and tests:
#   thetaDecoder / thetaRefMapper
#   → Ref.S
#   → PG-QP force planner
#   → tau = J^T f
#
# It should behave similarly to B5-c, but with iterative projected-gradient
# solving rather than one-shot LS + projection.
...
parser = argparse.ArgumentParser(description="CARTO Spot projected-gradient forceMPC debug")
...
parser.add_argument("--pg_iters", type=int, default=25)
parser.add_argument("--pg_step_size", type=float, default=0.08)
...
from isaaclab_carto.lowlevel.projected_gradient_force_mpc import (  # noqa: E402
    PGForceMPCState,
    make_projected_gradient_force_mpc_torque,
)
...
def print_debug(step, robot, u_cmd, beta_t, theta, ref, tau_cmd, info):
    ...
    print("\n" + "-" * 132)
    print(f"[PG FORCE MPC DEBUG] step={step}")
    print("-" * 132)
    ...
    keys = [
        ...
        "pg_num_iters", "pg_step_size", "pg_init",
    ...
    print("\n[tau_pg_force_mpc env0, action order]")
...
def main():
    ...
    planner_state = PGForceMPCState()

    print("\n" + "=" * 132)
    print("[INFO] Starting Spot B5-d projected-gradient forceMPC debug")
    print("=" * 132)
    print(f"z_mode={args.z_mode}, beta_preset={args.beta_preset}, u_cmd=[{args.vx},{args.vy},{args.wz}]")
    print(f"pg_iters={args.pg_iters}, pg_step_size={args.pg_step_size}, pg_init={args.pg_init}")
    print("=" * 132 + "\n")
    ...
    for step in range(args.steps):
        ...
        tau_cmd, info = make_projected_gradient_force_mpc_torque(
            robot=robot,
            ref=ref,
            x_hat=x_hat,
            u_cmd=u_cmd,
