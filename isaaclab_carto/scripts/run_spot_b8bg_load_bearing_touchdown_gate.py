...
#
# B8-bg: persistent foothold and load-bearing touchdown gate.
#
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B8-bg load-bearing touchdown gate")
...
def print_debug(step, phase, profile, phase_step, x_hat, base_ref, trial_out, ref_mask, contact_mask,
    ...
    print("\n" + "=" * 150)
    print(f"[B8-bg LOAD-BEARING-TOUCHDOWN-GATE] step={step}")
    print("=" * 150)
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B8-bg persistent foothold and load-bearing touchdown gate")
    ...
    for step in range(total_steps):
        ...
        if late_touchdown and args.foothold_accept_on_late_touchdown:
            # Reset selected joint target, but do not clear the foothold target yet
            # if load-bearing validation is enabled. Clearing too early caused the
            # lower/touchdown target to snap back to the anchor.
