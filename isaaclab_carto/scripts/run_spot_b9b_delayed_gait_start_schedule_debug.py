...
#
# B9-b: delayed gait start + schedule-aware debug.
#
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B9-b delayed gait start + schedule-aware debug")
...
def print_debug(step, phase, profile, phase_step, x_hat, base_ref, trial_out, ref_mask, contact_mask,
    ...
    print("\n" + "=" * 150)
    print(f"[B9-b DELAYED-GAIT-START-SCHEDULE-DEBUG] step={step}")
    print("=" * 150)
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B9-b delayed gait start + schedule-aware debug")
    ...
    for step in range(total_steps):
        ...
        if args.enable_canonical_gait_schedule:
            ...
            canonical_phi_t, canonical_s_t, canonical_sigma_t = compute_canonical_gait_schedule(step, args.num_envs, device, dtype)

            if canonical_prev_s_t is not None:
                liftoff = (canonical_prev_s_t > 0.5) & (canonical_s_t < 0.5)
                touchdown_edge = (canonical_prev_s_t < 0.5) & (canonical_s_t > 0.5)
                for li in range(4):
            ...
            canonical_prev_s_t = canonical_s_t.detach().clone()
