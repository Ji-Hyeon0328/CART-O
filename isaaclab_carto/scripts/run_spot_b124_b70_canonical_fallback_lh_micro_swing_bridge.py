...
parser.add_argument("--enable_b89_disable_b38_relocation_alignment", action="store_true", default=False)
...
# B120 CLEAN HIND-LEG POSITION-ONLY MICRO-SWING (B70+B98 ONLY)
# Goal: first make LH hind leg visibly lift/swing without raw torque compensation.
# Keep implicit PD as the actuator interface, but isolate the swing stack so B70/B98
# are the only active swing-position modifiers during the forced vertical pulse.
print('[B124 CONFIG] B123 + B70 canonical fallback: if LH latch opens but B23 has no committed target, B70 still drives a vertical-only micro-swing; no capture-hold')

# B123 note:
# B122 showed future_margin stalled around -0.065 while the relaxed threshold was -0.040.
# This run does not force liftoff; it relaxes the prepared-liftoff threshold to -0.070
# and gives B39/B11 more room to move base_ref.

# Force the first/only diagnostic swing to be LH, not RF.
args.test_leg = "LH"
args.step_order = "LH"
args.canonical_first_swing_leg = "LH"
args.canonical_crawl_priority = "LH,RH,RF,LF"
args.canonical_event_queue = "LH"
args.gait_phase_offsets = "LH:0.00,RF:0.25,RH:0.50,LF:0.75"
args.canonical_gait_start_step = 300
args.canonical_enable_event_crawl_queue = True
args.canonical_event_print = True

# B121: diagnostic forced LH liftoff gate.
# B120 never reached B70 because the future-support gate kept executed_s all-stance
# (future_not_allowed).  For this diagnostic we deliberately bypass that gate so
# the LH hind leg can actually enter active_swing.  This is NOT the final walking
# gate; it is only to verify whether the isolated B70/B98 position-only stack can
# visibly lift the hind leg once the gate opens.
# B122: do NOT force LH liftoff immediately.  First build support margin by
# shifting the trunk/base reference into the future 3-leg support region.
args.b10_allow_liftoff_on_b10_gate = False
args.b10_relocation_target_err_tol = 0.055
args.b10_relocation_geom_margin_tol = -0.035
args.b10_relocation_min_shift_steps = 240
args.b10_print = True
args.canonical_liftoff_min_margin = -0.050
args.canonical_liftoff_margin_relaxed = -0.060
args.canonical_liftoff_allow_after_shift_steps = 300

# Keep the diagnostic swing short.  We only want to see a hind-leg lift, not
# a full unstable step.
args.canonical_enable_latched_executed_swing = True
args.canonical_executed_swing_duration_steps = 120
args.canonical_executed_swing_min_duration_steps = 80
args.canonical_executed_swing_force_gap_after_touchdown = 160
args.canonical_print_latched_swing = True

# B124: B123 opened LH for one step, but B23 stayed inactive (no committed target),
# so B70 never received an active_swing target and the capture holder took over.
# Disable the capture holder and let B70 synthesize a vertical-only target directly
# from the canonical latched swing state.
args.enable_b124_b70_canonical_fallback = True
args.canonical_enable_touchdown_capture = False
args.canonical_capture_print = False
args.canonical_enable_touchdown_foothold_commit = False

# Keep stance stability from implicit PD, but do not fight it with raw torque compensation.
args.pd_scale = 0.35
args.enable_b78_true_all_stance_recovery = False
args.enable_b79_latched_recovery_posture = False

# Safe unload / forced vertical foot reference for LH only.
args.enable_b94_safe_pulse_hind_swing = True
args.b93_unload_steps = 360
args.b93_min_unload_alpha = 0.56
args.b93_release_alpha = 0.66
args.b93_min_unload_steps = 260
args.b93_hold_after_release_steps = 22
args.b93_min_future_margin = -0.095
args.b93_w_force_track = 10.0
args.b93_w_stance_acc = 22.0
...
args.b94_abort_roll = 0.050
args.b94_abort_pitch = 0.050

args.enable_b95_forced_hind_pulse_footref = True
args.b95_forward_step = 0.000
args.b95_lateral_step = 0.000
args.b95_lift_step = 0.024
args.b95_sigma = 0.10
args.b95_use_base_yaw = False
args.b95_print = True

# Main clean position adapter. Z-only, bounded, no torque assist.
args.enable_b70_minimal_clean_swing_adapter = True
args.b70_profile = "dls_safe"
args.b70_legs = "LH"
args.b70_gain = 0.08
args.b70_damping = 0.12
args.b70_clearance = 0.026
args.b70_min_clearance = 0.012
args.b70_max_joint_delta = 0.018
args.b70_max_hx_delta = 0.000
args.b70_max_hy_delta = 0.014
args.b70_max_kn_delta = 0.018
args.b70_err_scale_xy = 0.00
args.b70_err_scale_z = 0.80
args.b70_max_xy_err = 0.000
args.b70_max_z_err = 0.035
args.b70_apply_position_target = True
args.b70_apply_torque_assist = False
args.b70_zero_velocity = True
args.b70_print = True

# B98 empirical position-only lift inside B70.
# Use the milder dynamic candidate first: HY-, KN+.
# B106 said HY-/KN- gives stronger kinematic lift, but B119 showed it leaks into x-drift.
# HY-/KN+ should be gentler and may produce a visible lift with less x runaway.
args.enable_b98_vertical_lift_empirical_hykn = True
args.b98_hy_z_gain = -1.20
args.b98_kn_z_gain = 1.20
args.b98_z_deadband = 0.0015
args.b98_max_hy_delta = 0.014
args.b98_max_kn_delta = 0.018
args.b98_print = True

# Isolation during the forced vertical pulse.
...
args.enable_b76_walk_authority = False

# Disable overlapping swing stacks globally.  B23 is kept only as the forced-footref provider.
...
args.enable_b23_wbc_main_committed_footref = True
...
args.b63_enable_hind_capture_target_latch = False
args.enable_b64_active_swing_divergence_guard = True
args.b64_max_abs_z_err = 0.14
args.b64_max_foot_z = 0.20
...
# B120 final hard isolation after B68/B69 rewrites.
# Some older isolation wrappers re-enable B57/B62/B64; make the final state explicit.
if 'B120' in str(__file__) or True:
    args.test_leg = "LH"
    args.step_order = "LH"
    args.canonical_first_swing_leg = "LH"
    args.canonical_event_queue = "LH"
    ...
    args.enable_b57_arc_foothold_lock_bridge = False
    args.b57_apply_position_target = False
    args.b57_apply_torque_assist = False
    args.enable_b62_hind_empirical_arc_bridge = False
    args.enable_b45_directional_hind_y_push_bridge = False
    args.enable_b18_committed_foothold_pinning = False
    args.enable_b19_forced_exploratory_foothold_bias = False
    ...
    args.enable_b99_pure_vertical_lift_isolation = True
    if bool(getattr(args, "b70_print", False)):
        print('[B120 FINAL ISOLATION]',
              'test_leg=', args.test_leg,
              'queue=', args.canonical_event_queue,
              'B20/B21/B24/B49/B57/B62/B45=',
              (args.enable_b20_committed_foothold_direct_swing_ik,
               args.enable_b21_cartesian_committed_swing_torque,
               args.enable_b24_wbc_swing_authority_boost,
               args.enable_b49_elevated_swing_tracking_reference,
               args.enable_b57_arc_foothold_lock_bridge,
               args.enable_b62_hind_empirical_arc_bridge,
               args.enable_b45_directional_hind_y_push_bridge),
              'B70 gains=', (args.b70_gain, args.b70_max_hy_delta, args.b70_max_kn_delta),
              'B98 gains=', (args.b98_hy_z_gain, args.b98_kn_z_gain))

# B123 relaxed support-pre-shift override.  B122 improved margin only to about -0.065
# and then got stuck because ref-error and relaxed margin were too strict.
# Keep forced liftoff OFF, but allow a prepared relaxed liftoff near -0.07 margin.
args.enable_b89_disable_b38_relocation_alignment = False
args.enable_b11_wbc_relocation_task = True
args.b11_reloc_kp_base_xy = 75.0
args.b11_reloc_kd_base_xy = 16.0
args.b11_reloc_w_base_acc = 80.0
args.b11_reloc_w_stance_acc = 175.0
args.b11_reloc_max_base_acc_lin = 2.0
args.enable_b12_pd_target_alignment = False
args.enable_b38_candidate_support_recenter = False
args.enable_b39_latched_support_recenter = True
args.b39_candidate_legs = 'LH'
args.b39_force_shift_even_if_future_bad = True
args.b39_max_step = 0.0020
args.b39_target_err_tol = 0.075
args.b39_geom_margin_tol = -0.060
args.b39_min_shift_steps = 300
args.b39_allow_liftoff_on_b39_gate = True
args.b39_print = True
args.canonical_enable_feasible_trunk_shift = True
args.canonical_shift_before_liftoff = True
args.canonical_shift_during_swing = False
args.canonical_shift_track_actual_base = True
args.canonical_shift_hold_when_ref_error = False
args.canonical_shift_max_base_ref_xy_err = 0.090
args.canonical_shift_max_step = 0.0016
args.canonical_shift_blend = 1.0
args.canonical_shift_forward_bias = 0.0
args.canonical_shift_lateral_bias = 0.024
args.canonical_event_shift_min_future_margin = -999.0
args.canonical_liftoff_min_margin = -0.050
args.canonical_liftoff_margin_relaxed = -0.060
args.canonical_liftoff_allow_after_shift_steps = 300
# Avoid violent diagnostic pulse; after gated liftoff, start with small lift.
args.b95_lift_step = 0.018
args.b95_sigma = 0.08
args.b93_release_alpha = 0.72
args.b93_hold_after_release_steps = 16
# Keep hind position command mild.
args.b98_hy_z_gain = -0.95
args.b98_kn_z_gain = 0.95
args.b98_max_hy_delta = 0.010
args.b98_max_kn_delta = 0.012
args.b70_max_hy_delta = 0.010
args.b70_max_kn_delta = 0.012
if bool(getattr(args, 'b70_print', False)):
    print('[B124 FINAL FALLBACK MICRO-SWING]',
          'B11=', args.enable_b11_wbc_relocation_task,
          'B39=', (args.enable_b39_latched_support_recenter, args.b39_force_shift_even_if_future_bad,
                  args.b39_target_err_tol, args.b39_geom_margin_tol, args.b39_min_shift_steps),
          'canonical_shift=', (args.canonical_enable_feasible_trunk_shift, args.canonical_shift_track_actual_base,
                              args.canonical_shift_max_step, args.canonical_shift_max_base_ref_xy_err),
          'liftoff_gate=', (args.b39_allow_liftoff_on_b39_gate, args.canonical_liftoff_min_margin,
                           args.canonical_liftoff_margin_relaxed, args.canonical_liftoff_allow_after_shift_steps))
...
def _b55_capture_touchdown_ik_bridge(robot, Jfeet_full, foot_pos_arg, b23_info, step):
    ...
    try:
        ...
        joint_ids = _b12_get_leg_joint_ids(leg)
        # B119 fix: PhysX floating-base Jacobian has 6 base columns before the 12 joint columns.
...
def _b70_minimal_clean_swing_adapter(robot, Jfeet_full, foot_pos_arg, b23_info, step):
    ...
    # B124 fallback: in the B123 run the LH contact/latch opened, but B23 stayed
    # inactive because there was no committed foothold target.  That meant B70
    # never ran, and the post-touchdown/capture holder took over.  For this
    # diagnostic, if B23 is inactive but a canonical latched swing leg exists,
    # synthesize a minimal active_swing target directly from the latched leg.
    if (not isinstance(b23_info, dict) or not bool(b23_info.get("active", False))):
        if bool(getattr(args, "enable_b124_b70_canonical_fallback", False)):
            try:
                _latched_leg = globals().get("canonical_latched_swing_leg", None)
                _start_step = int(globals().get("canonical_latched_swing_start_step", -1))
                if _latched_leg is not None and int(_latched_leg) >= 0 and int(step) >= _start_step:
                    _leg = int(_latched_leg)
                    _leg_name = ID_TO_LEG.get(_leg, str(_leg)).upper()
                    _dur = max(1, int(getattr(args, "canonical_executed_swing_duration_steps", 120)))
                    _sigma = max(0.0, min(1.0, float(int(step) - _start_step) / float(_dur)))
                    try:
                        _anchor_t = globals().get("canonical_foot_anchor_w", None)
                        if _anchor_t is not None:
                            _anchor_np = _anchor_t[0, _leg, :].detach().cpu().numpy()
                        else:
                            _anchor_np = foot_pos_arg[0, _leg, :].detach().cpu().numpy()
                    except Exception:
                        _anchor_np = foot_pos_arg[0, _leg, :].detach().cpu().numpy()
                    _committed_np = _anchor_np.copy()
                    b23_info = {
                        "active": True,
                        "reason": "active_swing",
                        "leg": _leg_name,
                        "sigma": _sigma,
                        "anchor": _anchor_np,
                        "committed": _committed_np,
                        "target_after": _committed_np,
                        "b124_fallback": True,
                    }
                    info["reason"] = "b124_canonical_fallback"
                else:
                    info["reason"] = "no_active_b23_no_latch"
                    return q_target, tau, info
            except Exception as _b124_e:
                info["reason"] = f"b124_fallback_error:{repr(_b124_e)}"
                return q_target, tau, info
        else:
            info["reason"] = "no_active_b23"
            return q_target, tau, info
    ...
    try:
        ...
        joint_ids = _b12_get_leg_joint_ids(leg)
        # B119 fix: PhysX floating-base Jacobian has 6 base columns before joint columns.
        jac_joint_ids = [int(j) + 6 for j in joint_ids]
        J_leg = Jfeet_full[0, leg, :, jac_joint_ids]
        eye = torch.eye(J_leg.shape[0], device=J_leg.device, dtype=J_leg.dtype)
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B123 RELAXED PRE-SHIFT LH LIFTOFF CLEAN POSITION-ONLY MICRO-SWING planner")
