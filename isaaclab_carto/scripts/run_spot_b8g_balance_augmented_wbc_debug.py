# isaaclab_carto/scripts/run_spot_b8g_balance_augmented_wbc_debug.py
#
# B8-g: balance-augmented QPS-WBC bridge.
#
# Uses reduced implicit PD, but adds base height / pitch stabilization by
# redistributing vertical force over stance legs.
#
# This is still not walking.
# Goal:
#   In pd_scale around 0.5, avoid sit-down while keeping residual authority.
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B8-g balance-augmented QPS-WBC debug")

parser.add_argument("--num_envs", type=int, default=1)
parser.add_argument("--settle_steps", type=int, default=120)
parser.add_argument("--probe_steps", type=int, default=180)
...
parser.add_argument("--kp_h_balance", type=float, default=120.0)
parser.add_argument("--kd_h_balance", type=float, default=20.0)
parser.add_argument("--kp_pitch_balance", type=float, default=35.0)
parser.add_argument("--kd_pitch_balance", type=float, default=6.0)
parser.add_argument("--max_extra_fz_per_leg", type=float, default=8.0)
parser.add_argument("--max_remove_fz_per_leg", type=float, default=4.0)
parser.add_argument("--max_pitch_moment", type=float, default=8.0)
...
parser.add_argument("--w_swing", type=float, default=0.20)
parser.add_argument("--w_posture", type=float, default=0.35)
parser.add_argument("--w_reg", type=float, default=0.03)
parser.add_argument("--w_rate", type=float, default=0.20)

parser.add_argument("--kp_swing_xy", type=float, default=15.0)
parser.add_argument("--kp_swing_z", type=float, default=25.0)
parser.add_argument("--kd_swing_xy", type=float, default=1.5)
parser.add_argument("--kd_swing_z", type=float, default=2.0)
parser.add_argument("--max_task_cmd", type=float, default=3.0)
parser.add_argument("--max_pos_err", type=float, default=0.04)

parser.add_argument("--kp_posture", type=float, default=1.0)
parser.add_argument("--kd_posture", type=float, default=0.05)
parser.add_argument("--max_posture_tau", type=float, default=0.25)

# Authority
parser.add_argument("--tau_probe_scale", type=float, default=6.0)
...
def print_probe_debug(
    ...
    print("\n" + "=" * 132)
    print(f"[B8-g BALANCE-AUGMENTED QPS-WBC DEBUG] probe_step={probe_step}")
    print("=" * 132)
    ...
    for key in [
        ...
        "balance_vz_env0", "balance_pitch_rate_env0",
        "Fz_balance_env0", "My_balance_env0",
        "x_rel_env0", "fz_bias_env0",
        "f_bias_norm_env0", "f_aug_norm_env0",
    ]:
        if key in wbc_info:
    ...
    print("\n[authority probe with balance augmentation]")
    print("pd_scale:", args.pd_scale)
...
def main():
    ...
    print("\n" + "=" * 132)
    print("[INFO] Starting B8-g balance-augmented QPS-WBC debug")
    print("=" * 132)
    ...
    for probe_step in range(args.probe_steps):
        ...
        tau_raw, wbc_info, _f_aug = make_balance_augmented_qps_wbc_torque(
            robot=robot,
            ref=ref,
            x_hat=x_hat,
            f_ref=f_ref,
