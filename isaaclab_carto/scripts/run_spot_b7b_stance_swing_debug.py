# isaaclab_carto/scripts/run_spot_b7b_stance_swing_debug.py
#
# B7-b: stance force residual + swing tracking residual together.
#
# Uses EffortAction env:
#
#   tau_total =
#       tau_stance_pg_force_mpc_v2
#       + swing_scale * tau_swing_pd
#
# Current status:
#   - Still not WBC
#   - Still uses Isaac implicit PD as posture stabilizer
#   - Intended to verify interaction between stance support residual and swing tracking residual
#
# If robot sits/falls:
#   reduce swing_scale, kp_swing, hy/kn deltas
#   or use conservative only.
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B7-b stance force + swing tracking debug")
...
# Swing effort tracking
parser.add_argument("--enable_swing", action="store_true")
parser.add_argument("--swing_scale", type=float, default=0.5)
parser.add_argument("--kp_swing", type=float, default=8.0)
parser.add_argument("--kd_swing", type=float, default=0.6)
parser.add_argument("--max_swing_tau", type=float, default=0.8)
parser.add_argument("--hy_lift_delta", type=float, default=-0.025)
parser.add_argument("--kn_lift_delta", type=float, default=0.060)
parser.add_argument("--hy_sweep_delta", type=float, default=0.005)
...
from isaaclab_carto.lowlevel.swing_tracking_effort import make_swing_tracking_torque  # noqa: E402
...
def print_debug(step, robot, u_cmd, beta_t, theta, ref, tau_stance, tau_swing, tau_total, stance_info, swing_info):
    env_id = 0
    x_hat_after = make_x_hat(robot, velocity_frame="world")

    print("\n" + "=" * 132)
    print(f"[B7-b STANCE + SWING DEBUG] step={step}")
    print("=" * 132)
    ...
    print("tau_swing stats:", summarize_torque(tau_swing))

    print("\n[tau total env0, action order]")
...
def main():
    ...
    stance_state = PGForceMPCV2State()

    print("\n" + "=" * 132)
    print("[INFO] Starting B7-b stance force + swing tracking debug")
    print("=" * 132)
    print(f"z_mode={args.z_mode}, beta_preset={args.beta_preset}, u_cmd=[{args.vx},{args.vy},{args.wz}]")
    print(f"enable_swing={args.enable_swing}, swing_scale={args.swing_scale}")
    print("NOTE: this is still not WBC; it is residual stance force + residual swing PD.")
    print("=" * 132 + "\n")
    ...
    for step in range(args.steps):
        ...
        tau_stance, stance_info = make_pg_force_mpc_v2_torque(
            robot=robot,
            ref=ref,
            x_hat=x_hat,
            u_cmd=u_cmd,
            beta_t=beta_t,
            planner_state=stance_state,
            h_ref=args.h_ref,
            mass=args.mass,
        ...
        tau_swing, swing_info = make_swing_tracking_torque(
            robot=robot,
            ref=ref,
            theta=theta,
            k=args.ref_k,
            kp_swing=args.kp_swing,
            kd_swing=args.kd_swing,
            max_swing_tau=args.max_swing_tau,
        ...
        if not args.enable_swing:
            tau_swing = torch.zeros_like(tau_swing)

        tau_total = tau_stance + args.swing_scale * tau_swing
        tau_total = torch.clamp(tau_total, -args.max_total_tau, args.max_total_tau)
