# isaaclab_carto/scripts/run_spot_b8ac_full_wbc_qp_v0.py
import os, sys, argparse
from typing import Any
import torch
from isaaclab.app import AppLauncher

parser = argparse.ArgumentParser(description="CARTO/TRACER B8-ac full-body WBC-QP v0")
...
parser.add_argument("--w_dyn", type=float, default=20.0)
parser.add_argument("--w_base_acc", type=float, default=5.0)
parser.add_argument("--w_stance_acc", type=float, default=30.0)
parser.add_argument("--w_swing_acc", type=float, default=40.0)
parser.add_argument("--w_force_track", type=float, default=1.0)
parser.add_argument("--w_swing_force_zero", type=float, default=50.0)
parser.add_argument("--w_tau_posture", type=float, default=0.10)
parser.add_argument("--w_tau_reg", type=float, default=0.02)
parser.add_argument("--w_qdd_reg", type=float, default=0.002)
parser.add_argument("--kp_base_xy", type=float, default=12.0)
parser.add_argument("--kd_base_xy", type=float, default=8.0)
parser.add_argument("--kp_base_z", type=float, default=40.0)
parser.add_argument("--kd_base_z", type=float, default=10.0)
parser.add_argument("--kp_base_rp", type=float, default=30.0)
parser.add_argument("--kd_base_rp", type=float, default=6.0)
parser.add_argument("--kp_base_yaw", type=float, default=10.0)
parser.add_argument("--kd_base_yaw", type=float, default=3.0)
parser.add_argument("--swing_clearance", type=float, default=0.060)
parser.add_argument("--kp_swing_z", type=float, default=180.0)
parser.add_argument("--kd_swing_z", type=float, default=20.0)
parser.add_argument("--kp_swing_xy", type=float, default=0.0)
parser.add_argument("--kd_swing_xy", type=float, default=0.0)
parser.add_argument("--max_swing_acc", type=float, default=15.0)
...
parser.add_argument("--max_tau", type=float, default=28.0)
...
from isaaclab_carto.lowlevel.tracer_full_wbc_qp_v0 import FullWbcQpV0Config, solve_full_wbc_qp_v0
...
def print_debug(step, phase, profile, x_hat, base_ref, out, stance_mask, f_mpc, wrench, foot_pos, foot0, foot_swing0, swing_target, tau, qpd, robot, swing_enabled):
    ...
    print("\n" + "="*132)
    print(f"[B8-ac FULL-WBC-QP-V0] step={step}")
    print("="*132)
    ...
    print("swing_acc_des LF/RF/LH/RH:"); print(qpd["swing_acc_des"][0].detach().cpu().numpy())
...
def main():
    ...
    ref_cfg = SupportRegionRefConfig(alpha=args.alpha, margin=args.margin,
                                     max_shift_per_step=args.max_shift_per_step,
                                     height_ref=args.height_ref)
    mpc_cfg = MpcWbcBridgeConfig(kp_xy=args.kp_xy, kd_xy=args.kd_xy, kp_z=args.kp_z,
                                 kd_z=args.kd_z, mass=mass, mu=args.mu,
                                 min_fz=args.min_fz, max_fz=args.max_fz)
    wbc_cfg = FullWbcQpV0Config(w_dyn=args.w_dyn, w_base_acc=args.w_base_acc,
                                w_stance_acc=args.w_stance_acc, w_swing_acc=args.w_swing_acc,
                                w_force_track=args.w_force_track, w_swing_force_zero=args.w_swing_force_zero,
                                w_tau_posture=args.w_tau_posture, w_tau_reg=args.w_tau_reg,
                                w_qdd_reg=args.w_qdd_reg, kp_base_xy=args.kp_base_xy,
                                kd_base_xy=args.kd_base_xy, kp_base_z=args.kp_base_z,
                                kd_base_z=args.kd_base_z, kp_base_rp=args.kp_base_rp,
                                kd_base_rp=args.kd_base_rp, kp_base_yaw=args.kp_base_yaw,
                                kd_base_yaw=args.kd_base_yaw, kp_swing_xy=args.kp_swing_xy,
                                kd_swing_xy=args.kd_swing_xy, kp_swing_z=args.kp_swing_z,
                                kd_swing_z=args.kd_swing_z, max_swing_acc=args.max_swing_acc,
                                kp_posture=args.kp_posture, kd_posture=args.kd_posture,
                                max_posture_tau=args.max_posture_tau, mu=args.mu,
                                min_fz=0.0, max_fz=args.max_fz, max_tau=args.max_tau,
    ...
    print("\n" + "="*132)
    print("[INFO] Starting B8-ac quasi-static full-body WBC-QP v0")
    ...
    for step in range(total_steps):
        ...
        f_mpc, wrench = distribute_grf_ls(x_hat[:,0:3], x_hat[:,3:6], x_hat[:,6:9],
                                          x_hat[:,9:12], base_ref, foot_pos, stance_mask, mpc_cfg)
        swing_target = make_swing_target(phase, profile, foot_pos, foot_swing0, swing_enabled)

        tau, qpd = solve_full_wbc_qp_v0(M, Jfeet_full, f_mpc, robot.data.joint_pos, robot.data.joint_vel,
                                        q_nom, x_hat, base_ref, foot_pos, foot_vel, swing_target,
                                        stance_mask, gravity, coriolis, wbc_cfg)
        env.step(tau)
        if step % max(args.print_every, 1) == 0:
            print_debug(step, phase, profile, x_hat, base_ref, out, stance_mask, f_mpc, wrench,
                        foot_pos, foot0, foot_swing0, swing_target, tau, qpd, robot, swing_enabled)
