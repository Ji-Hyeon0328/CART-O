...
#
# B8-bk: delayed touchdown until foothold commit.
#
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B8-bk delayed touchdown until foothold commit")
...
def print_debug(step, phase, profile, phase_step, x_hat, base_ref, trial_out, ref_mask, contact_mask,
    ...
    print("\n" + "=" * 150)
    print(f"[B8-bk DELAYED-TOUCHDOWN-FOOTHOLD-COMMIT] step={step}")
    print("=" * 150)
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B8-bk delayed touchdown until foothold commit")
    ...
    for step in range(total_steps):
        ...
        # B8-bi: the previous swing touchdown/recovery is an explicit sub-state.
        # While it is active, do not let the nominal schedule's step_idx/active_leg
        # overwrite previous_swing_leg, previous_swing_stance_q_ref, or the foothold target.
        ...
        if freeze_step_key_for_touchdown:
            ...
            if current_step_key is not None and args.enable_real_touchdown_gate:
                ...
                previous_swing_foothold_target_w = foothold_target_w.detach().clone() if foothold_target_w is not None else None
                previous_swing_stance_q_ref = current_swing_stance_q_ref.detach().clone() if current_swing_stance_q_ref is not None else None
                touchdown_gate_hold_count = 0
                touchdown_load_hold_count = 0
                touchdown_load_diag = None
                touchdown_recovery_frozen_key = current_step_key
            ...
            if step_idx >= 1 and not recenter_active:
                # If recenter was not safe, do not silently start the next swing.
                pass
            foot_swing0 = None
            foothold_target_w = None
            foothold_target_leg = None
        ...
        if late_touchdown_candidate and args.defer_contact_restore_until_foothold_valid:
            _td_leg = LEG_TO_ID[args.test_leg] # type: ignore
        ...
        if late_touchdown and args.foothold_accept_on_late_touchdown:
            ...
            if not args.enable_touchdown_stance_recovery:
                reset_selected_joint_targets_to_current(robot, LEG_TO_ID[args.test_leg], zero_velocity=args.touchdown_reset_velocity_zero) # type: ignore
            ...
            if args.anchor_swing_on_release:
                if swing_enabled:
                    ...
                    if args.enable_explicit_foothold_target:
                        foothold_target_leg = LEG_TO_ID[args.test_leg] # type: ignore
        ...
        pre_touchdown_lock_target, pre_touchdown_lock_delta_full, pre_touchdown_lock_info = apply_foothold_lock_recovery_target(
            robot, Jfeet_full, foot_pos, LEG_TO_ID[args.test_leg], foothold_target_w, # type: ignore
