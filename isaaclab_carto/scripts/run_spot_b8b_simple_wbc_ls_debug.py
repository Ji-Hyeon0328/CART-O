# isaaclab_carto/scripts/run_spot_b8b_simple_wbc_ls_debug.py
#
# B8-b: Simple WBC-LS style torque debug.
#
# Pipeline:
#   thetaDecoder / thetaRefMapper
#   → forceMPC v2 f_ref
#   → simple WBC-LS:
#       tau = J_stance^T f_ref
#           + J_swing^T f_swing_task
#           + tau_posture
#
# This is still not full dynamics QP WBC, but swing tracking is now task-space.
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B8-b simple WBC-LS debug")
...
# Simple WBC-LS terms
parser.add_argument("--stance_force_scale", type=float, default=1.0)
parser.add_argument("--swing_task_scale", type=float, default=0.20)
...
parser.add_argument("--max_swing_force", type=float, default=4.0)
parser.add_argument("--max_pos_err", type=float, default=0.05)

parser.add_argument("--disable_posture", action="store_true")
...
from isaaclab_carto.lowlevel.simple_wbc_ls import make_simple_wbc_ls_torque  # noqa: E402
...
def print_debug(step, robot, u_cmd, beta_t, theta, ref, f_ref, force_info, tau_total, wbc_info):
    ...
    print("\n" + "=" * 132)
    print(f"[B8-b SIMPLE WBC-LS DEBUG] step={step}")
    print("=" * 132)
    ...
    for key in [
        ...
        "swing_pos_err_env0",
        "f_swing_env0",
        "tau_stance_mean_abs", "tau_stance_max_abs",
        "tau_swing_mean_abs", "tau_swing_max_abs",
        "tau_posture_mean_abs", "tau_posture_max_abs",
        "tau_total_mean_abs", "tau_total_max_abs",
    ]:
        if key in wbc_info:
...
def main():
    ...
    print("\n" + "=" * 132)
    print("[INFO] Starting B8-b simple WBC-LS debug")
    print("=" * 132)
    print(f"z_mode={args.z_mode}, beta_preset={args.beta_preset}, u_cmd=[{args.vx},{args.vy},{args.wz}]")
    print("NOTE: This is task-space swing + stance force, not full dynamics QP WBC.")
    print("=" * 132 + "\n")
    ...
    for step in range(args.steps):
        ...
        tau_total, wbc_info = make_simple_wbc_ls_torque(
            robot=robot,
            ref=ref,
            f_ref=f_ref,
            k=args.ref_k,
            ...
            swing_task_scale=args.swing_task_scale,
            stance_force_scale=args.stance_force_scale,
            enable_posture=not args.disable_posture,
