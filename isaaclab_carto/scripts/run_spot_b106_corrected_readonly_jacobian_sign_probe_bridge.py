...
# B106 CORRECTED READ-ONLY JACOBIAN HY/KN SIGN PROBE OVERRIDES
...
# B106: corrected read-only kinematic sign probe. Do not command any joint target.
# Uses root-physx Jacobian columns with floating-base offset: joint_col = joint_id + 6.
...
print('[B106 CONFIG] corrected read-only Jacobian HY/KN sign probe: uses floating-base offset joint columns (joint_id + 6); no joint target injection, no contact release')
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B106 CORRECTED READ-ONLY JACOBIAN HY/KN SIGN PROBE planner")
    ...
    for step in range(total_steps):
        ...
        # B106: corrected read-only kinematic HY/KN sign probe.
        ...
        if bool(getattr(args, "enable_b104_readonly_kinematic_hykn_sign_probe", False)):
            try:
                ...
                if _do_print:
                    ...
                    _source = "none"
                    try:
                        # B106 correction from B105 audit:
                        # root_physx_view.get_jacobians() has 18 columns:
                        #   0:6   floating-base DoFs
                        #   6:18  actuated joint DoFs in robot joint-id order.
                        # Therefore use joint_col = joint_id + 6 instead of raw joint_id.
                        _jac_all = robot.root_physx_view.get_jacobians()
                        _body_row = int(foot_indices[_leg])  # hl_foot row is 15 for LH in this asset.
                        _cols = [int(_hx_id + 6), int(_hy_id + 6), int(_kn_id + 6)]
                        _J_leg = _jac_all[0, _body_row, 0:3, _cols].detach()
                        _source = f"root_physx_view body_row={_body_row} cols={_cols} joint_id_plus_6"
                    except Exception:
                        _J_leg = None
                    if _J_leg is None:
                        b104_readonly_probe_info = {"active": False, "reason": "no_jacobian", "leg": _leg_name}
                        if bool(getattr(args, "b104_print", True)):
                            print("[B106 CORRECTED READONLY JACOBIAN HY/KN SIGN PROBE]",
                ...
                if bool(getattr(args, "b104_print", True)):
                    print("[B106 CORRECTED READONLY JACOBIAN HY/KN SIGN PROBE ERROR]", "step=", step, "error=", repr(_b104_e))
