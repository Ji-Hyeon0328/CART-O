...
#
# B8-bb: per-step swing activation fix + bounded touchdown.
#
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B8-bb per-step swing activation fix + bounded touchdown")
...
# B8-bb: ensure selected leg in step_idx>=1 can actually enter swing/IK.
parser.add_argument("--enable_per_step_swing_force", action="store_true")
parser.add_argument("--per_step_force_from_step_idx", type=int, default=1)
parser.add_argument("--per_step_force_after_phase_step", type=int, default=20)
parser.add_argument("--per_step_force_max_abs_roll", type=float, default=0.22)
parser.add_argument("--per_step_force_max_abs_pitch", type=float, default=0.22)
parser.add_argument("--per_step_force_min_base_z", type=float, default=0.60)
...
def per_step_swing_force_gate(phase, phase_step, step_idx, x_hat):
    if not args.enable_per_step_swing_force:
        return False
    if step_idx < args.per_step_force_from_step_idx:
        return False
    if phase not in ["lift", "hold_lift", "lower"]:
        return False
    if phase_step < args.per_step_force_after_phase_step:
        return False
    ...
    if abs(roll) > args.per_step_force_max_abs_roll:
        return False
    if abs(pitch) > args.per_step_force_max_abs_pitch:
        return False
    if base_z < args.per_step_force_min_base_z:
        return False
    return True
...
def print_debug(step, phase, profile, phase_step, x_hat, base_ref, trial_out, ref_mask, contact_mask,
    ...
    print("\n" + "=" * 150)
    print(f"[B8-bb PER-STEP-SWING-ACTIVATION] step={step}")
    print("=" * 150)
    ...
    print("old_forced:", old_forced, "controlled_forced:", controlled_forced,
          "per_step_forced:", per_step_forced,
...
def main():
    ...
    if args.enable_two_step_sequence:
        total_steps = args.warmup_steps + one_step_len() * len(parse_step_order())

    print("\n" + "=" * 150)
    print("[INFO] Starting B8-bb per-step swing activation fix + bounded touchdown")
    ...
    print("per_step_swing_force:",
          "enabled:", args.enable_per_step_swing_force,
          "from_step_idx:", args.per_step_force_from_step_idx,
          "after_phase_step:", args.per_step_force_after_phase_step,
          "max_roll:", args.per_step_force_max_abs_roll,
          "max_pitch:", args.per_step_force_max_abs_pitch,
          "min_base_z:", args.per_step_force_min_base_z)
    ...
    for step in range(total_steps):
        ...
        # B8-ba: before starting the second step, require the previous swing foot to be physically down.
        ...
        if args.enable_real_touchdown_gate and args.enable_two_step_sequence and step_idx >= 1:
            if previous_swing_leg is not None:
                ...
                if (not touchdown_gate_down) or touchdown_gate_hold_count < args.touchdown_hold_steps:
                    phase = "settle"
                    profile = 0.0
                    phase_step = touchdown_gate_hold_count
                    active_leg = args.test_leg
                    touchdown_gate_active = True
                    touchdown_gate_hold_count += 1
                    if touchdown_gate_down and args.reset_joint_target_on_touchdown:
        ...
        if args.auto_select_second_leg and args.enable_two_step_sequence and step_idx == 1 and not touchdown_gate_active:
        ...
        if step_key != current_step_key:
            if current_step_key is not None and args.enable_real_touchdown_gate:
                # Keep checking the leg that just finished swinging before the next step starts.
                previous_swing_leg = LEG_TO_ID.get(str(current_step_key[1]), None)
                touchdown_gate_hold_count = 0
            foot_swing0 = None
            swing_anchor_step = -1
            if args.reset_base_ref_on_step_change:
                prev_base_ref = None
        ...
        ref_mask = all_stance_mask(args.num_envs, device, dtype) if phase in ["warmup", "settle", "done"] or late_touchdown else future_swing_mask(args.num_envs, device, dtype)
        ...
        per_step_forced = per_step_swing_force_gate(phase, phase_step, step_idx, x_hat)
        if per_step_forced:
            swing_enabled = True

        if phase in ["shift", "settle", "warmup", "done"] or late_touchdown or touchdown_gate_active:
            # pre-shift / settle / warmup / late-touchdown / touchdown-gate keep all contacts
            swing_enabled = False
            per_step_forced = False
        ...
        if q_target is None:
            q_target = ik_target if ik_target is not None else crouch_target
