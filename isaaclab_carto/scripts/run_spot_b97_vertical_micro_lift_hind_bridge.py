...
# B97 VERTICAL MICRO-LIFT HIND OVERRIDES
# B96 finally connected LH pulse release -> forced footref -> B70 active=True,
# but the forward+up target created too much base velocity and growing foot error.
# B97 keeps the successful B96 gate path, but removes almost all forward motion
# and tests only a short, safe vertical micro-lift.
...
print('[B97 CONFIG] vertical-only LH/RH micro-lift: B95 pulse presented as active_swing for B70')
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B97 VERTICAL MICRO-LIFT HIND + B96/B95/B94 recovery planner")
        ...
        try:
            ...
            if _b95_active:
                ...
                if bool(getattr(args, "b95_print", True)) and (step == b95_pulse_start_step or step % max(args.print_every, 1) == 0):
                    print("[B97/B95 FORCED VERTICAL MICRO-LIFT FOOTREF]",
                          "step=", step,
                ...
                if bool(getattr(args, "enable_b95_forced_hind_pulse_footref", False)) and b95_pulse_leg is not None:
                    if bool(getattr(args, "b95_print", True)) and step % max(args.print_every, 1) == 0:
                        print("[B97/B95 FORCED VERTICAL MICRO-LIFT FOOTREF END]", "step=", step, "leg=", ID_TO_LEG.get(int(b95_pulse_leg), b95_pulse_leg))
                b95_pulse_leg = None
                b95_pulse_anchor_w = None
                b95_pulse_target_w = None
                b95_pulse_start_step = -1
            ...
            if bool(getattr(args, "b95_print", True)) and step % max(args.print_every, 1) == 0:
                print("[B97/B95 FORCED VERTICAL MICRO-LIFT FOOTREF ERROR]", "step=", step, "error=", repr(_b95_e))
