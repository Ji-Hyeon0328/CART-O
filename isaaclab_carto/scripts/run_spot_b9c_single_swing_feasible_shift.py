...
#
# B9-c: single-swing crawl + feasibility-inspired trunk shift.
#
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B9-c single-swing crawl + feasibility-inspired trunk shift")
...
def apply_canonical_execution_gate(
    ...
    if new_liftoff and int(all_stance_gap_count) > 0:
        ...
        if new_liftoff and (not allowed or not margin_ok or not roll_ok or not pitch_ok or not z_ok):
            executed[:] = 1.0
            current_swing_leg = None
            if not allowed:
...
def apply_canonical_feasible_trunk_shift(base_ref, x_hat, foot_pos, exec_info, prev_base_ref):
...
def print_debug(step, phase, profile, phase_step, x_hat, base_ref, trial_out, ref_mask, contact_mask,
    ...
    print("\n" + "=" * 150)
    print(f"[B9-c SINGLE-SWING-FEASIBLE-SHIFT] step={step}")
    print("=" * 150)
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B9-c single-swing crawl + feasibility-inspired trunk shift")
    ...
    for step in range(total_steps):
        ...
        if args.enable_canonical_gait_schedule and args.canonical_gait_disable_event_overrides:
            ...
            if canonical_exec_prev_s_t is not None:
                ...
                if args.canonical_gait_print_edges and bool((liftoff | touchdown_edge).any().detach().cpu()):
                    print("[B9-c EDGE] step=", step,
                          ...
                          "requested_s=", canonical_requested_s_t[0].detach().cpu().numpy(), # type: ignore # type: ignore
            ...
            base_ref, canonical_shift_info = apply_canonical_feasible_trunk_shift(
                base_ref, x_hat, foot_pos, canonical_exec_info, prev_base_ref
            )
            base_ref_raw = base_ref.detach().clone()
