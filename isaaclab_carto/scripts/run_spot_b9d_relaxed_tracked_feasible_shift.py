...
#
# B9-d: relaxed gate + tracked feasible trunk shift.
#
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B9-d relaxed gate + tracked feasible trunk shift")
...
def make_base_ref(x_hat, foot_pos, ref_mask, prev_base_ref, cfg, phase, phase_step, active_leg=None):
    ...
    base_ref[:, 3:5] = 0.0
    out.fr_lite_info = fr_lite_info # type: ignore
    return base_ref, out
...
def print_debug(step, phase, profile, phase_step, x_hat, base_ref, trial_out, ref_mask, contact_mask,
    ...
    print("\n" + "=" * 150)
    print(f"[B9-d RELAXED-TRACKED-FEASIBLE-SHIFT] step={step}")
    print("=" * 150)
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B9-d relaxed gate + tracked feasible trunk shift")
    ...
    for step in range(total_steps):
        ...
        if args.enable_canonical_gait_schedule and args.canonical_gait_disable_event_overrides:
            ...
            if canonical_shift_info.get("active", False):
                ...
                if not bool((canonical_requested_s_t[0] < 0.5).any().detach().cpu()): # type: ignore
                    canonical_shift_hold_count = 0
                    canonical_last_shift_target_xy = None
            canonical_exec_info["shift_hold_count"] = int(canonical_shift_hold_count)
            canonical_exec_info["base_ref_xy_err"] = (
                float(canonical_shift_info.get("base_ref_xy_err", torch.zeros((1,), device=device))[0].detach().cpu()) # type: ignore
        ...
        if args.enable_canonical_gait_schedule and args.canonical_gait_disable_event_overrides:
            ...
            swing_enabled = bool((canonical_s_t < 0.5).any().detach().cpu()) # type: ignore
        ...
        if committed_pin_active:
            committed_pin_target, committed_pin_delta_full, committed_pin_info = apply_committed_foothold_pin_target(
                robot, Jfeet_full, foot_pos, int(touchdown_committed_leg), touchdown_committed_foothold_target_w, True # type: ignore
            )
        ...
        if step % max(args.print_every, 1) == 0:
            if args.enable_canonical_gait_schedule and canonical_s_t is not None:
                ...
                print("[B9-c canonical]",
                      ...
                      "executed_s=", canonical_s_t[0].detach().cpu().numpy(),
                      "sigma=", canonical_sigma_t[0].detach().cpu().numpy(), # type: ignore
                      "contact_mask=", contact_mask[0].detach().cpu().numpy(),
                      "gate_reason=", canonical_exec_info.get("gate_reason", None),
                      "candidate=", ID_TO_LEG.get(canonical_exec_info.get("candidate_leg"), None) if canonical_exec_info.get("candidate_leg") is not None else None, # type: ignore
                      ...
                      "shift_next=", canonical_shift_info.get("next_xy", None)[0].detach().cpu().numpy() if canonical_shift_info.get("next_xy", None) is not None else None, # type: ignore
                      "shift_target=", canonical_shift_info.get("target_xy", None)[0].detach().cpu().numpy() if canonical_shift_info.get("target_xy", None) is not None else None) # type: ignore
