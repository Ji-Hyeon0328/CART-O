...
#
# B9-m: stance-IK pre-shift adapter.
#
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B9-m stance-IK pre-shift adapter")
...
# B9-m: convert feasible trunk-shift reference into stance-leg IK/torque action.
parser.add_argument("--canonical_enable_stance_ik_preshift", action="store_true")
parser.add_argument("--canonical_stance_ik_gain", type=float, default=0.75)
parser.add_argument("--canonical_stance_ik_damping", type=float, default=0.040)
parser.add_argument("--canonical_stance_ik_max_joint_delta", type=float, default=0.050)
parser.add_argument("--canonical_stance_ik_max_foot_xy_cmd", type=float, default=0.030)
parser.add_argument("--canonical_stance_ik_include_hx", action="store_true")
parser.add_argument("--canonical_stance_ik_sign", type=float, default=-1.0)
parser.add_argument("--canonical_stance_ik_kp", type=float, default=42.0)
parser.add_argument("--canonical_stance_ik_kd", type=float, default=2.5)
parser.add_argument("--canonical_stance_ik_max_tau", type=float, default=5.0)
parser.add_argument("--canonical_stance_ik_print", action="store_true")
...
def apply_canonical_stance_ik_preshift(robot, Jfeet_full, x_hat, base_ref, foot_pos, contact_mask, shift_info):
    """B9-m: realize feasibility pre-shift through stance-leg IK torques.

    B9-l could compute a nearby feasible target, but the body did not move because
    only base_ref changed. In all-stance, to move the trunk while keeping stance
    feet fixed, command the stance legs to move their feet in the opposite direction
    relative to the body. Contact then converts that internal leg motion into trunk
    motion.
    """
    q_now = robot.data.joint_pos.detach()
    qd_now = robot.data.joint_vel.detach()
    tau_add = torch.zeros_like(q_now)
    q_delta_full = torch.zeros_like(q_now)
    q_target_full = None
    ...
    info = {
        "active": False,
        "candidate_leg": None,
        "base_err_xy": torch.zeros((B, 2), device=device, dtype=dtype),
        "foot_cmd_xy": torch.zeros((B, 2), device=device, dtype=dtype),
        "tau_add_max": 0.0,
        "delta_max": 0.0,
        "joint_ids": [],
    }

    if not args.canonical_enable_stance_ik_preshift:
        return None, q_delta_full, tau_add, info
    if not shift_info.get("active", False):
        return None, q_delta_full, tau_add, info
    if not bool(shift_info.get("shift_before", False)):
        return None, q_delta_full, tau_add, info
    ...
    # Desired trunk motion from current base projection toward the feasibility reference.
    # We limit this command so it becomes a local pre-shift action rather than a jump.
    base_err_xy = base_ref[:, 0:2] - x_hat[:, 0:2]
    norm = torch.linalg.norm(base_err_xy, dim=1, keepdim=True).clamp_min(1.0e-9)
    max_cmd = float(args.canonical_stance_ik_max_foot_xy_cmd)
    trunk_cmd_xy = base_err_xy / norm * torch.clamp(norm, max=max_cmd)

    # To move the body toward +trunk_cmd while stance feet stay in contact,
    # request the feet to move opposite relative to the body.
    foot_cmd_xy = float(args.canonical_stance_ik_sign) * trunk_cmd_xy
    ...
    for leg in range(4):
        ...
        if contact_mask is not None:
            try:
                if float(contact_mask[0, leg].detach().cpu()) < 0.5:
                    continue
            except Exception:
                pass

        if args.canonical_stance_ik_include_hx:
            joint_ids = [int(HX[leg]), int(HY[leg]), int(KN[leg])]
        else:
            joint_ids = [int(HY[leg]), int(KN[leg])]
        ...
        err_use = foot_cmd_xy
        dq = damped_least_squares_delta(J_use, err_use, float(args.canonical_stance_ik_damping))
        dq = float(args.canonical_stance_ik_gain) * dq
        dq = torch.clamp(dq, -float(args.canonical_stance_ik_max_joint_delta), float(args.canonical_stance_ik_max_joint_delta))
        ...
        tau_leg = float(args.canonical_stance_ik_kp) * (q_selected - q_now[:, joint_ids]) - float(args.canonical_stance_ik_kd) * qd_now[:, joint_ids]
        tau_leg = torch.clamp(tau_leg, -float(args.canonical_stance_ik_max_tau), float(args.canonical_stance_ik_max_tau))
        tau_add[:, joint_ids] += tau_leg
        all_joint_ids.extend(joint_ids)

    if len(all_joint_ids) == 0:
        return None, q_delta_full, tau_add, info

    # Also update implicit position targets for environments where the actuator model reads them.
    joint_ids_unique = sorted(set(all_joint_ids))
    joint_ids_tensor = torch.tensor(joint_ids_unique, device=device, dtype=torch.long)
    robot.set_joint_position_target(q_target_full[:, joint_ids_unique], joint_ids=joint_ids_tensor)
    robot.set_joint_velocity_target(torch.zeros_like(q_target_full[:, joint_ids_unique]), joint_ids=joint_ids_tensor)

    info["active"] = True
    info["candidate_leg"] = candidate
    info["base_err_xy"] = base_err_xy.detach().clone()
    info["foot_cmd_xy"] = foot_cmd_xy.detach().clone()
    info["tau_add_max"] = float(tau_add.abs().max().detach().cpu())
    info["delta_max"] = float(q_delta_full.abs().max().detach().cpu())
    info["joint_ids"] = joint_ids_unique
    return q_target_full, q_delta_full, tau_add, info
...
def print_debug(step, phase, profile, phase_step, x_hat, base_ref, trial_out, ref_mask, contact_mask,
    ...
    print("\n" + "=" * 150)
    print(f"[B9-m STANCE-IK-PRESHIFT] step={step}")
    print("=" * 150)
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B9-m stance-IK pre-shift adapter")
    ...
    print("stance_ik_preshift:",
          "enabled:", args.canonical_enable_stance_ik_preshift,
          "gain:", args.canonical_stance_ik_gain,
          "max_joint_delta:", args.canonical_stance_ik_max_joint_delta,
          "max_foot_xy_cmd:", args.canonical_stance_ik_max_foot_xy_cmd,
          "sign:", args.canonical_stance_ik_sign,
          "kp:", args.canonical_stance_ik_kp,
          "kd:", args.canonical_stance_ik_kd,
          "max_tau:", args.canonical_stance_ik_max_tau)
    print("=" * 150)
    ...
    for step in range(total_steps):
            ...
            if args.canonical_enable_latched_executed_swing:
                ...
                if canonical_latched_swing_leg is None and int(opened_ids.numel()) > 0:
                    ...
                    if args.canonical_print_latched_swing:
                        print("[B9-m LATCH START] step=", step, "leg=", ID_TO_LEG[canonical_latched_swing_leg])
                ...
                if canonical_latched_swing_leg is not None:
                    ...
                    if canonical_latched_swing_elapsed < duration:
                        ...
                        if args.canonical_print_latched_swing:
                            print("[B9-m LATCH END] step=", step, "leg=", ID_TO_LEG[canonical_latched_swing_leg])
                        canonical_s_t = torch.ones_like(canonical_s_t)
                        canonical_sigma_t = torch.zeros_like(canonical_sigma_t)
                        canonical_current_swing_leg = None
                        ...
                        if args.canonical_enable_touchdown_capture:
                            ...
                            if args.canonical_capture_print:
                                print("[B9-m CAPTURE START] step=", step,
        ...
        stance_preshift_target, stance_preshift_delta_full, stance_preshift_tau, stance_preshift_info = apply_canonical_stance_ik_preshift(
            robot, Jfeet_full, x_hat, base_ref, foot_pos, contact_mask, canonical_shift_info
        )
        if stance_preshift_info.get("active", False):
            tau_cmd = tau_cmd + stance_preshift_tau
            if args.canonical_stance_ik_print and (step % max(args.print_every, 1) == 0):
                print("[B9-m STANCE IK PRESHIFT]",
                      "step=", step,
                      "candidate=", ID_TO_LEG.get(stance_preshift_info.get("candidate_leg"), None),
                      "base_err_xy=", stance_preshift_info["base_err_xy"][0].detach().cpu().numpy(),
                      "foot_cmd_xy=", stance_preshift_info["foot_cmd_xy"][0].detach().cpu().numpy(),
                      "delta_max=", stance_preshift_info["delta_max"],
                      "tau_add_max=", stance_preshift_info["tau_add_max"])
        ...
        if canonical_capture_info.get("active", False):
            ...
            if canonical_capture_count <= 0:
                if args.canonical_capture_print:
                    print("[B9-m CAPTURE END] step=", step, "leg=", ID_TO_LEG.get(canonical_capture_leg, None))
        ...
        if args.enable_canonical_gait_schedule and args.canonical_gait_disable_event_overrides:
            ...
            ik_target = None
            crouch_target = None
            # Keep B9-m stance pre-shift target for logging and actuator target path.
            q_target = stance_preshift_target if stance_preshift_target is not None else None
                    ...
                    if pre_touchdown_lock_target is not None
                        ...
                        if foothold_lock_target is not None
                            ...
                            if touchdown_recovery_target is not None
                                ...
                                stance_preshift_target
                                if stance_preshift_target is not None
