...
parser.add_argument("--enable_b78_true_all_stance_recovery", action="store_true", default=False)
...
parser.add_argument("--b78_print", action="store_true", default=False)
...
parser.add_argument("--enable_b79_latched_recovery_posture", action="store_true", default=False)
...
parser.add_argument("--b79_print", action="store_true", default=False)
...
# B108 CORRECTED JACOBIAN HY-/KN- LIFT PULSE + SMALL TORQUE ASSIST OVERRIDES
...
args.b93_hold_after_release_steps = 10
args.b93_min_future_margin = -0.085
...
args.b94_abort_base_z = 0.650
args.b94_abort_roll = 0.055
args.b94_abort_pitch = 0.055
...
args.b95_lift_step = 0.014
args.b95_sigma = 0.12
args.b95_use_base_yaw = False
args.b95_print = True
# Make B70 gentler during the pulse so we can observe controlled lift instead of kick.
args.b70_gain = 0.08
args.b70_damping = 0.08
args.b70_max_joint_delta = 0.007
args.b70_max_hx_delta = 0.004
args.b70_max_hy_delta = 0.006
args.b70_max_kn_delta = 0.006
args.b70_err_scale_xy = 0.10
args.b70_err_scale_z = 0.30
args.b70_max_xy_err = 0.012
args.b70_max_z_err = 0.022
args.b70_z_only_projection = True
args.b70_z_only_use_hx = False
# B108: position-only did not lift the foot in B107. Keep the same tiny q_cmd
# but add a strictly capped HY/KN torque assist through the B70 torque path.
# Disable B76 torque selection so B70's small max_tau is used, while B99 still mutes
# the larger B74/B75/B76 auxiliary controllers during the vertical pulse.
args.b70_apply_torque_assist = False
args.b70_torque_kp = 0.0
args.b70_torque_kd = 0.0
args.b70_max_tau = 0.0
# B112: clean 4-case feedforward torque sign sweep.
# Keep the B110 injection path, but choose the HY/KN sign by elapsed pulse time.
args.enable_b110_sign_fixed_ff_torque = True
args.enable_b112_ff_torque_sign_sweep = True
args.b112_case_steps = 6
args.b112_gap_steps = 18
args.b112_tau_amp = 0.80
args.b112_cases = "HY-,KN+;HY+,KN-;HY-,KN-;HY+,KN+"
args.b112_only_during_forced_vertical_pulse = True
args.b112_print = True
args.b110_ff_tau_hx = 0.0
args.b110_ff_tau_hy = -0.80
args.b110_ff_tau_kn = -0.80
args.b110_ff_max_tau = 0.80
args.b110_only_during_forced_vertical_pulse = False
args.b110_print = True
args.b76_enable_small_torque = False
args.b76_max_tau = 1.50
# Make the forced contact-open window long enough to test all four cases.
args.b93_hold_after_release_steps = 96
# This is a torque-only sign sweep; do not let position target/B98 contaminate the sign test.
args.b70_apply_position_target = False
...
args.b98_hy_z_gain = 0.0
args.b98_kn_z_gain = 0.0
args.b98_z_deadband = 0.002
args.b98_max_hy_delta = 0.0
args.b98_max_kn_delta = 0.0
args.b98_print = False
...
args.b99_print = True
print('[B114 CONFIG] raw effort authority probe: all-stance, no swing, LH HY/KN effort override after WBC/B22; logs q/qd/foot/applied response')
print('[B112 RECOVERY DISABLED] enable_b78=', bool(getattr(args, 'enable_b78_true_all_stance_recovery', False)), 'enable_b79=', bool(getattr(args, 'enable_b79_latched_recovery_posture', False)), 'b78_print=', bool(getattr(args, 'b78_print', False)), 'b79_print=', bool(getattr(args, 'b79_print', False)))
print('[B112 SWEEP] cases=', getattr(args, 'b112_cases', None), 'case_steps=', getattr(args, 'b112_case_steps', None), 'gap_steps=', getattr(args, 'b112_gap_steps', None), 'tau_amp=', getattr(args, 'b112_tau_amp', None), 'q_target=', bool(getattr(args, 'b70_apply_position_target', True)), 'b98=', bool(getattr(args, 'enable_b98_vertical_lift_empirical_hykn', False)))

# B114: raw-effort authority probe.  This intentionally bypasses the swing pipeline.
# Keep the robot all-stance, run WBC for general standing, then overwrite only the
# selected LH HY/KN (and HX=0) effort entries immediately before env.step().
args.enable_b114_raw_effort_authority_probe = True
args.enable_b115_fixed_raw_effort_probe = True
args.enable_b116_action_path_audit = True
# B118: keep implicit PD for standing, but compensate its reported residual on the probed leg.
args.enable_b118_implicit_pd_compensation = True
args.b118_comp_gain = 1.0
args.b118_comp_clip = 22.0
args.b118_desired_is_applied_torque = True
print('[B118 COMPENSATION ENABLED] pd_scale=', args.pd_scale, 'gain=', args.b118_comp_gain, 'clip=', args.b118_comp_clip)
# B115: hard-disable all residual swing/assist bridges that can contaminate raw effort authority probing.
args.enable_b62_hind_empirical_arc_bridge = False
args.enable_b45_directional_hind_y_push_bridge = False
args.enable_b63_hind_capture_target_latch = False
args.enable_b64_active_swing_divergence_guard = False
args.enable_b18_committed_foothold_pinning = False
args.enable_b19_forced_exploratory_foothold_bias = False
args.enable_b23_wbc_main_committed_footref = False
args.enable_b24_wbc_swing_authority_boost = False
args.enable_b48_post_landing_feasibility_foothold_planner = False
args.enable_b49_elevated_swing_tracking_reference = False
args.enable_b90_soft_margin_builder = False

args.b114_probe_leg = 'LH'
args.b114_start_step = 180
args.b114_case_steps = 16
args.b114_gap_steps = 24
args.b114_cases = 'HY-2,KN-2;HY+2,KN+2;HY-5,KN-5;HY+5,KN+5;HY-10,KN-10;HY+10,KN+10;HY-5,KN+5;HY+5,KN-5'
args.b114_zero_hx = True
args.b114_override_hykn = True
args.b114_print = True
# Stop scheduled/canonical swing so B114 is an all-stance effort authority test.
...
args.enable_b70_minimal_clean_swing_adapter = False
args.enable_b110_sign_fixed_ff_torque = False
args.enable_b112_ff_torque_sign_sweep = False
args.enable_b98_vertical_lift_empirical_hykn = False
args.enable_b99_pure_vertical_lift_isolation = False
# Disable extra swing/foothold torque sources; leave WBC standing as the stabilizer.
args.enable_b18_committed_foothold_pinning = False
args.enable_b20_committed_foothold_direct_swing_ik = False
args.enable_b21_cartesian_committed_swing_torque = False
args.enable_b57_arc_foothold_lock_bridge = False
args.enable_b68_swing_assist_isolation_probe = False
args.enable_b69_true_wbc_only_isolation_probe = False
args.enable_b78_true_all_stance_recovery = False
args.enable_b79_latched_recovery_posture = False
args.b78_print = False
args.b79_print = False
print('[B116 ACTION PATH AUDIT PROBE] leg=', args.b114_probe_leg, 'start=', args.b114_start_step, 'case_steps=', args.b114_case_steps, 'gap_steps=', args.b114_gap_steps, 'cases=', args.b114_cases)
...
def _b55_capture_touchdown_ik_bridge(robot, Jfeet_full, foot_pos_arg, b23_info, step):
    ...
    try:
        ...
        joint_ids = _b12_get_leg_joint_ids(leg)
        # B107 fix: PhysX floating-base Jacobian has 6 base columns before the 12 joint columns.
        # Joint ids [hx,hy,kn] must therefore be read as [id+6].
        jac_joint_ids = [int(j) + 6 for j in joint_ids]
        J_leg = Jfeet_full[0, leg, :, jac_joint_ids]
...
def _b55_print_info(step, info):
    ...
    if isinstance(info, dict) and (info.get("active", False) or step % max(args.print_every, 1) == 0):
        print("[B55 CAPTURE TOUCHDOWN IK BRIDGE]",
              ...
              "jac_joint_ids=", info.get("jac_joint_ids"),
...
def _b57_arc_foothold_lock_bridge(robot, Jfeet_full, foot_pos_arg, b23_info, step):
    ...
    try:
        ...
        info.update({
            ...
            "jac_joint_ids": locals().get("jac_joint_ids", None),
...
# ==================================================================================================
# B114 raw effort authority probe helpers
# ==================================================================================================
def _b114_parse_case(case_name: str):
    hy_tau = 0.0
    kn_tau = 0.0
    for part in str(case_name).split(','):
        p = part.strip().upper()
        if p.startswith('HY'):
            sign = -1.0 if '-' in p[:3] else 1.0
            mag = ''.join(ch for ch in p if (ch.isdigit() or ch == '.'))
            hy_tau = sign * float(mag or 0.0)
        elif p.startswith('KN'):
            sign = -1.0 if '-' in p[:3] else 1.0
            mag = ''.join(ch for ch in p if (ch.isdigit() or ch == '.'))
            kn_tau = sign * float(mag or 0.0)
    return hy_tau, kn_tau


def _b114_raw_effort_override(robot, tau_cmd, foot_pos, step):
    info = {"active": False, "reason": "disabled", "case": None, "case_name": None, "elapsed": None,
            "tau_override": None, "q_leg": None, "qd_leg": None, "foot_w": None,
            "foot_delta": None, "base_z": None, "base_rpy": None}
    if not bool(getattr(args, 'enable_b114_raw_effort_authority_probe', False)):
        return tau_cmd, info
    leg_name = str(getattr(args, 'b114_probe_leg', 'LH')).upper()
    leg_id = LEG_TO_ID.get(leg_name, 2)
    joint_ids = _b12_get_leg_joint_ids(leg_id)
    start = int(getattr(args, 'b114_start_step', 760))
    elapsed_total = int(step) - start
    if elapsed_total < 0:
        info.update({"reason": "before_start", "elapsed": elapsed_total})
        return tau_cmd, info
    cases = [c.strip() for c in str(getattr(args, 'b114_cases', 'HY-2,KN-2;HY+2,KN+2')).split(';') if c.strip()]
    case_steps = max(int(getattr(args, 'b114_case_steps', 20)), 1)
    gap_steps = max(int(getattr(args, 'b114_gap_steps', 40)), 0)
    block = case_steps + gap_steps
    case_idx = elapsed_total // block
    in_case_step = elapsed_total % block
    if case_idx >= len(cases):
        info.update({"reason": "done", "elapsed": elapsed_total})
        return tau_cmd, info
    case_name = cases[int(case_idx)]
    if in_case_step >= case_steps:
        info.update({"reason": "gap", "case": int(case_idx), "case_name": case_name, "elapsed": int(in_case_step)})
        return tau_cmd, info
    hy_tau, kn_tau = _b114_parse_case(case_name)
    tau_new = tau_cmd.detach().clone()
    desired_tau_leg = torch.zeros(3, device=tau_cmd.device, dtype=tau_cmd.dtype)
    if bool(getattr(args, 'b114_zero_hx', True)):
        desired_tau_leg[0] = 0.0
    else:
        desired_tau_leg[0] = tau_cmd[0, int(joint_ids[0])]
    desired_tau_leg[1] = float(hy_tau)
    desired_tau_leg[2] = float(kn_tau)

    # B118: With implicit actuator PD enabled, applied_torque ~= action + implicit_drive_residual.
    # Estimate that residual from the previous reported applied torque and the current nominal WBC command,
    # then command the residual-cancelled action so the *applied* torque approaches desired_tau_leg.
    implicit_est_leg = torch.zeros(3, device=tau_cmd.device, dtype=tau_cmd.dtype)
    comp_cmd_leg = desired_tau_leg.clone()
    if bool(getattr(args, 'enable_b118_implicit_pd_compensation', False)):
        try:
            applied_prev_leg_t = robot.data.applied_torque[0, joint_ids].detach().to(device=tau_cmd.device, dtype=tau_cmd.dtype)
            nominal_action_leg_t = tau_cmd[0, joint_ids].detach()
            implicit_est_leg = applied_prev_leg_t - nominal_action_leg_t
            comp_gain = float(getattr(args, 'b118_comp_gain', 1.0))
            comp_cmd_leg = desired_tau_leg - comp_gain * implicit_est_leg
            comp_clip = float(getattr(args, 'b118_comp_clip', float(args.max_tau)))
            comp_cmd_leg = torch.clamp(comp_cmd_leg, -comp_clip, comp_clip)
        except Exception as _comp_e:
            print('[B118 COMPENSATION WARN]', 'step=', step, 'err=', repr(_comp_e))
            comp_cmd_leg = desired_tau_leg.clone()

    if bool(getattr(args, 'b114_zero_hx', True)):
        tau_new[0, int(joint_ids[0])] = comp_cmd_leg[0]
    if bool(getattr(args, 'b114_override_hykn', True)):
        tau_new[0, int(joint_ids[1])] = comp_cmd_leg[1]
        tau_new[0, int(joint_ids[2])] = comp_cmd_leg[2]
    else:
        tau_new[0, int(joint_ids[1])] += comp_cmd_leg[1]
        tau_new[0, int(joint_ids[2])] += comp_cmd_leg[2]
    # latch a baseline foot/q at first activation for delta logging
    global b114_anchor_foot_w, b114_anchor_q_leg
    try:
        b114_anchor_foot_w
    except NameError:
        b114_anchor_foot_w = None
        b114_anchor_q_leg = None
    if b114_anchor_foot_w is None:
        b114_anchor_foot_w = foot_pos[0, int(leg_id)].detach().clone()
        b114_anchor_q_leg = robot.data.joint_pos[0, joint_ids].detach().clone()
    foot_w = foot_pos[0, int(leg_id)].detach().clone()
    q_leg = robot.data.joint_pos[0, joint_ids].detach().clone()
    qd_leg = robot.data.joint_vel[0, joint_ids].detach().clone()
    try:
        rpy = quat_to_rpy(robot.data.root_quat_w.detach())[0].detach().cpu().numpy()
    except Exception:
        rpy = None
    info.update({
        "active": True, "reason": "case", "case": int(case_idx), "case_name": case_name,
        "elapsed": int(in_case_step), "leg": leg_name, "joint_ids": [int(x) for x in joint_ids],
        "desired_applied_tau": desired_tau_leg.detach().cpu().numpy(),
        "implicit_est_leg": implicit_est_leg.detach().cpu().numpy(),
        "tau_override": tau_new[0, joint_ids].detach().cpu().numpy(),
        "tau_wbc_before": tau_cmd[0, joint_ids].detach().cpu().numpy(),
        "action_before_override": tau_cmd[0, joint_ids].detach().cpu().numpy(),
        "action_after_override": tau_new[0, joint_ids].detach().cpu().numpy(),
        "q_leg": q_leg.detach().cpu().numpy(), "qd_leg": qd_leg.detach().cpu().numpy(),
        "q_delta": (q_leg - b114_anchor_q_leg).detach().cpu().numpy() if b114_anchor_q_leg is not None else None,
        "foot_w": foot_w.detach().cpu().numpy(),
        "foot_delta": (foot_w - b114_anchor_foot_w).detach().cpu().numpy() if b114_anchor_foot_w is not None else None,
        "base_z": float(robot.data.root_pos_w[0,2].detach().cpu()), "base_rpy": rpy,
    })
    return torch.clamp(tau_new, -float(args.max_tau), float(args.max_tau)), info


def _b114_print_info(step, info, robot):
    if not bool(getattr(args, 'enable_b114_raw_effort_authority_probe', False)):
        return
    if not bool(getattr(args, 'b114_print', True)):
        return
    should = bool(info.get('active', False)) or (step % max(int(getattr(args, 'print_every', 20)), 1) == 0)
    if not should:
        return
    leg_name = str(getattr(args, 'b114_probe_leg', 'LH')).upper()
    leg_id = LEG_TO_ID.get(leg_name, 2)
    joint_ids = _b12_get_leg_joint_ids(leg_id)
    try:
        applied = robot.data.applied_torque[0, joint_ids].detach().cpu().numpy()
    except Exception:
        applied = None
    print('[B118 COMPENSATED ACTION AUDIT PRE]',
          'step=', step,
          ...
          'joint_ids=', info.get('joint_ids'),
          'tau_wbc_before=', info.get('tau_wbc_before'),
          'action_before_override=', info.get('action_before_override'),
          'desired_applied_tau=', info.get('desired_applied_tau'),
          'implicit_est_leg=', info.get('implicit_est_leg'),
          'tau_override=', info.get('tau_override'),
          'action_after_override=', info.get('action_after_override'),
          'applied_prev=', applied,
          'q_leg=', info.get('q_leg'),
          'qd_leg=', info.get('qd_leg'),
          'q_delta=', info.get('q_delta'),
          'foot_w=', info.get('foot_w'),
          'foot_delta=', info.get('foot_delta'),
          'base_z=', info.get('base_z'),
          'base_rpy=', info.get('base_rpy'))


def _b116_print_post_step(step, pre_info, action_sent, robot):
    """Print what the simulator reports after env.step(action_sent)."""
    if not bool(getattr(args, 'enable_b116_action_path_audit', False)):
        return
    if not isinstance(pre_info, dict) or not bool(pre_info.get('active', False)):
        return
    leg_name = str(getattr(args, 'b114_probe_leg', 'LH')).upper()
    leg_id = LEG_TO_ID.get(leg_name, 2)
    joint_ids = _b12_get_leg_joint_ids(leg_id)
    try:
        ...
        applied = None
    try:
        q_leg = robot.data.joint_pos[0, joint_ids].detach().cpu().numpy()
        qd_leg = robot.data.joint_vel[0, joint_ids].detach().cpu().numpy()
    except Exception:
        q_leg, qd_leg = None, None
    try:
        action_sent_leg = action_sent[0, joint_ids].detach().cpu().numpy()
    except Exception:
        action_sent_leg = None
    try:
        foot_w = robot.data.body_pos_w[0, int(foot_indices[int(leg_id)]), :].detach().cpu().numpy()
    except Exception:
        foot_w = None
    try:
        base_z = float(robot.data.root_pos_w[0, 2].detach().cpu())
        rpy = quat_to_rpy(robot.data.root_quat_w.detach())[0].detach().cpu().numpy()
    except Exception:
        base_z, rpy = None, None
    print('[B118 COMPENSATED ACTION AUDIT POST]',
          'step=', step,
          'case=', pre_info.get('case'),
          'case_name=', pre_info.get('case_name'),
          'elapsed=', pre_info.get('elapsed'),
          'joint_ids=', [int(x) for x in joint_ids],
          'action_sent_to_env=', action_sent_leg,
          'applied_after_step=', applied,
          'applied_minus_action=', (applied - action_sent_leg) if applied is not None and action_sent_leg is not None else None,
          'desired_applied_tau=', pre_info.get('desired_applied_tau'),
          'applied_minus_desired=', (applied - pre_info.get('desired_applied_tau')) if applied is not None and pre_info.get('desired_applied_tau') is not None else None,
          'q_after=', q_leg,
          'qd_after=', qd_leg,
          'foot_w_after=', foot_w,
          'base_z_after=', base_z,
...
def _b70_minimal_clean_swing_adapter(robot, Jfeet_full, foot_pos_arg, b23_info, step):
    ...
    info = {
        ...
        "tau_pd": None, "tau_ff": None, "tau_total": None, "b112_case": None, "b112_case_name": None, "b112_elapsed": None,
    }
    ...
    try:
        ...
        joint_ids = _b12_get_leg_joint_ids(leg)
        # B107 fix: PhysX floating-base Jacobian has 6 base columns before joint columns.
        jac_joint_ids = [int(j) + 6 for j in joint_ids]
        J_leg = Jfeet_full[0, leg, :, jac_joint_ids]
        damp = float(getattr(args, "b70_damping", 0.070))
        if bool(getattr(args, "b70_z_only_projection", False)):
            # B109: for the vertical pulse, do not let the DLS solution spend
            # authority on x/y or HX.  Solve only the scalar z row for HY/KN.
            # This keeps corrected joint_id+6 columns but projects the command
            # into a pure lift subspace.
            if bool(getattr(args, "b70_z_only_use_hx", False)):
                cols_local = [0, 1, 2]
            else:
                cols_local = [1, 2]
            Jz = J_leg[2:3, cols_local]  # shape (1, n)
            ez = err_scaled[2:3]
            denom = (Jz @ Jz.transpose(0, 1)) + (damp ** 2)
            dq_sub = Jz.transpose(0, 1) @ (ez / denom.reshape(()))
            dq = torch.zeros(3, device=J_leg.device, dtype=J_leg.dtype)
            for _ii, _col in enumerate(cols_local):
                dq[_col] = dq_sub[_ii]
        else:
            eye = torch.eye(J_leg.shape[0], device=J_leg.device, dtype=J_leg.dtype)
            A = J_leg @ J_leg.transpose(0, 1) + (damp ** 2) * eye
        ...
        if bool(getattr(args, "b70_z_only_projection", False)) and bool(locals().get("b99_forced_vertical_pulse", False)):
            dq[0] = torch.zeros_like(dq[0])
        ...
        # B110: separate PD torque from a sign-fixed feedforward torque pulse.
        # This tests whether a direct HY-/KN- torque command can create lift, without
        # relying on q_cmd PD whose KN sign flipped in B109 due to velocity/current state.
        qd = robot.data.joint_vel.detach()
        if bool(getattr(args, "b70_apply_torque_assist", False)) or ((bool(getattr(args, "enable_b76_walk_authority", True)) and bool(getattr(args, "b76_enable_small_torque", True))) and not (bool(getattr(args, "enable_b99_pure_vertical_lift_isolation", False)) and bool(getattr(args, "b99_disable_b76_during_pulse", True)) and bool(locals().get("b99_forced_vertical_pulse", False)))):
            if bool(getattr(args, "enable_b76_walk_authority", True)) and bool(getattr(args, "b76_enable_small_torque", True)):
                ...
                max_tau_pd = float(getattr(args, "b76_max_tau", 4.5))
                if bool(locals().get("b76_emergency_active", False)):
                    max_tau_pd *= float(getattr(args, "b76_emergency_torque_scale", 1.25))
                b76_walk_authority_active = True
            else:
                kp_tau = float(getattr(args, "b70_torque_kp", 14.0))
                kd_tau = float(getattr(args, "b70_torque_kd", 1.5))
                max_tau_pd = float(getattr(args, "b70_max_tau", 3.0))
            tau_pd = kp_tau * (q_cmd - q_now[:, joint_ids]) - kd_tau * qd[:, joint_ids]
            tau_pd = torch.clamp(tau_pd, -max_tau_pd, max_tau_pd)
            b76_max_tau_used = max_tau_pd
        else:
            tau_pd = torch.zeros((q_now.shape[0], 3), device=q_now.device, dtype=q_now.dtype)
            max_tau_pd = 0.0

        tau_ff = torch.zeros_like(tau_pd)
        b110_active = bool(getattr(args, "enable_b110_sign_fixed_ff_torque", False))
        if b110_active and bool(getattr(args, "b110_only_during_forced_vertical_pulse", True)):
            b110_active = bool(locals().get("b99_forced_vertical_pulse", False))
        b112_case_idx = None
        b112_case_name = None
        b112_elapsed = None
        if b110_active and bool(getattr(args, "enable_b112_ff_torque_sign_sweep", False)):
            # B113: anchor sweep to the first B70 active-swing call, not to b95/b99 pulse.
            if not hasattr(args, "_b113_sweep_start_step"):
                setattr(args, "_b113_sweep_start_step", int(step))
            start = int(getattr(args, "_b113_sweep_start_step", int(step)))
            b112_elapsed = int(step) - start
            case_steps = max(int(getattr(args, "b112_case_steps", 6)), 1)
            gap_steps = max(int(getattr(args, "b112_gap_steps", 18)), 0)
            block = case_steps + gap_steps
            case_idx = int(b112_elapsed // block)
            within = int(b112_elapsed % block)
            cases = [c.strip() for c in str(getattr(args, "b112_cases", "HY-,KN+;HY+,KN-;HY-,KN-;HY+,KN+")).split(';') if c.strip()]
            if 0 <= case_idx < len(cases) and within < case_steps:
                b112_case_idx = case_idx
                b112_case_name = cases[case_idx]
                amp = float(getattr(args, "b112_tau_amp", 0.8))
                hy_sign = -1.0 if 'HY-' in b112_case_name else 1.0
                kn_sign = -1.0 if 'KN-' in b112_case_name else 1.0
                ff = torch.tensor([[0.0, hy_sign * amp, kn_sign * amp]], device=q_now.device, dtype=q_now.dtype)
                ff_cap = float(getattr(args, "b110_ff_max_tau", amp))
                tau_ff = torch.clamp(ff, -ff_cap, ff_cap)
            else:
                # Gap between cases, intentionally no FF torque.
                b110_active = False
        elif b110_active:
            ff = torch.tensor([[float(getattr(args, "b110_ff_tau_hx", 0.0)),
                                float(getattr(args, "b110_ff_tau_hy", -0.8)),
                                float(getattr(args, "b110_ff_tau_kn", -0.8))]], device=q_now.device, dtype=q_now.dtype)
            ff_cap = float(getattr(args, "b110_ff_max_tau", 0.8))
            tau_ff = torch.clamp(ff, -ff_cap, ff_cap)
        tau_leg = tau_pd + tau_ff
        total_cap = max(float(getattr(args, "b110_ff_max_tau", 0.8)), float(max_tau_pd))
        if total_cap > 0.0:
            tau_leg = torch.clamp(tau_leg, -total_cap, total_cap)
        tau[:, joint_ids] = tau_leg
        ...
        info.update({
            ...
            "tau_leg": tau_leg[0].detach().cpu().numpy(),
            "tau_pd": tau_pd[0].detach().cpu().numpy(),
            "tau_ff": tau_ff[0].detach().cpu().numpy(),
            "tau_total": tau_leg[0].detach().cpu().numpy(),
            "b112_case": b112_case_idx,
            "b112_case_name": b112_case_name,
            "b112_elapsed": b112_elapsed,
            "jac_joint_ids": locals().get("jac_joint_ids", None),
...
def _b70_print_info(step, info):
    ...
    if isinstance(info, dict) and (info.get("active", False) or step % max(args.print_every, 1) == 0):
        print("[B70 MINIMAL CLEAN SWING ADAPTER]",
              ...
              "clamped=", info.get("clamped"),
              "tau_leg=", info.get("tau_leg"),
              "tau_pd=", info.get("tau_pd"),
              "tau_ff=", info.get("tau_ff"),
              "tau_total=", info.get("tau_total"),
              "b112_case=", info.get("b112_case"),
              "b112_case_name=", info.get("b112_case_name"),
              "b112_elapsed=", info.get("b112_elapsed"),
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B118 IMPLICIT-PD-COMPENSATED RAW EFFORT PROBE (ALL-STANCE, LH HY/KN OVERRIDE) planner")
    ...
    for step in range(total_steps):
        ...
        b114_raw_info = {"active": False, "reason": "not_called"}
        b116_action_sent_to_env = None
        try:
            tau_cmd, b114_raw_info = _b114_raw_effort_override(robot, tau_cmd, foot_pos, step)
            _b114_print_info(step, b114_raw_info, robot)
            b116_action_sent_to_env = tau_cmd.detach().clone()
        except Exception as _b114_e:
            print('[B118 COMPENSATED ACTION AUDIT ERROR]', 'step=', step, 'err=', repr(_b114_e))
            b116_action_sent_to_env = tau_cmd.detach().clone()
        ...
        try:
            _b116_print_post_step(step, b114_raw_info, b116_action_sent_to_env, robot)
        except Exception as _b116_post_e:
            print('[B118 COMPENSATED ACTION AUDIT POST ERROR]', 'step=', step, 'err=', repr(_b116_post_e))
