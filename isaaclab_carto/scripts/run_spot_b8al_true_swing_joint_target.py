...
#
# B8-al: Stable B8-ah rollback + true selected swing-joint target diagnostic.
#
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B8-al true selected swing-joint target diagnostic")
...
# B8-al: true selected-joint target assist.
# This uses joint_ids=[selected swing leg HY/KN] and avoids setting a full target vector.
parser.add_argument("--swing_joint_target_assist_only", action="store_true")
parser.add_argument("--swing_joint_target_base", type=str, default="release", choices=["release", "current", "nominal"])
parser.add_argument("--swing_joint_target_hx_offset", type=float, default=0.0)
parser.add_argument("--swing_joint_target_hy_offset", type=float, default=0.0)
parser.add_argument("--swing_joint_target_kn_offset", type=float, default=0.0)
parser.add_argument("--swing_joint_target_start_profile", type=float, default=0.20)
parser.add_argument("--swing_joint_target_max_delta", type=float, default=0.08)
parser.add_argument("--swing_joint_target_use_hx", action="store_true")
...
def apply_true_swing_joint_target(robot, q_nom, q_release, phase, profile, swing_enabled):
    """Set implicit position target only for selected swing-leg joints.

    This is the key difference from B8-aj:
      B8-aj called set_joint_position_target(full_q_target)
      B8-al calls set_joint_position_target(q_selected, joint_ids=[...])

    Therefore stance-leg targets are not explicitly overwritten.
    """
    q_target_full = None
    q_delta_full = torch.zeros_like(robot.data.joint_pos.detach())

    if not args.swing_joint_target_assist_only:
        return None, q_delta_full
    if not swing_enabled:
        return None, q_delta_full
    if phase not in ["lift", "hold_lift", "lower"]:
        return None, q_delta_full
    if profile < args.swing_joint_target_start_profile:
        return None, q_delta_full

    q_now = robot.data.joint_pos.detach()
    if args.swing_joint_target_base == "current":
        base_q = q_now.clone()
    elif args.swing_joint_target_base == "nominal":
        base_q = q_nom.clone()
    else:
        base_q = q_release.clone() if q_release is not None else q_now.clone()

    leg = LEG_TO_ID[args.test_leg]
    denom = max(1e-6, 1.0 - args.swing_joint_target_start_profile)
    s = float(max(0.0, min(1.0, (profile - args.swing_joint_target_start_profile) / denom)))

    joint_ids = []
    offsets = []
    if args.swing_joint_target_use_hx or abs(args.swing_joint_target_hx_offset) > 1e-12:
        joint_ids.append(int(HX[leg]))
        offsets.append(args.swing_joint_target_hx_offset)
    joint_ids.append(int(HY[leg]))
    offsets.append(args.swing_joint_target_hy_offset)
    joint_ids.append(int(KN[leg]))
    offsets.append(args.swing_joint_target_kn_offset)

    q_selected = base_q[:, joint_ids].clone()
    for k, off in enumerate(offsets):
        off = max(-args.swing_joint_target_max_delta, min(args.swing_joint_target_max_delta, float(off)))
        q_selected[:, k] = q_selected[:, k] + off * s
        q_delta_full[:, joint_ids[k]] = q_selected[:, k] - q_now[:, joint_ids[k]]

    # True selected-joint target call.
    joint_ids_tensor = torch.tensor(joint_ids, device=q_selected.device, dtype=torch.long)
    ...
    return q_target_full, q_delta_full
...
def make_swing_target(phase, profile, foot_pos, foot_swing0, swing_enabled):
    ...
    if phase in ["lift", "hold_lift", "lower"] and swing_enabled:
        if args.target_z_mode == "step":
            z_offset = clearance
        elif args.target_z_mode == "hold":
            z_offset = 0.0
        else:
            z_offset = clearance * profile
...
def print_debug(step, phase, profile, phase_step, x_hat, base_ref, trial_out, ref_mask, contact_mask,
                ...
                swing_anchor_step, q_delta_full):
    leg = LEG_TO_ID[args.test_leg]
    ...
    print("\n" + "=" * 150)
    print(f"[B8-al TRUE-SWING-JOINT-TARGET] step={step}")
    print("=" * 150)
    ...
    print("swing_joint_target_assist_only:", args.swing_joint_target_assist_only,
          "joint_target_base:", args.swing_joint_target_base,
          "hy_offset:", args.swing_joint_target_hy_offset,
          "kn_offset:", args.swing_joint_target_kn_offset)
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B8-al true selected swing-joint target diagnostic")
    ...
    for step in range(total_steps):
        ...
        if foot_swing0 is None:
            if args.anchor_swing_on_release:
                if swing_enabled:
                    ...
                    q_release = robot.data.joint_pos.detach().clone()
        ...
        q_delta_full = torch.zeros_like(robot.data.joint_pos.detach())
        if (not args.align_targets) and args.swing_joint_target_assist_only:
            q_target, q_delta_full = apply_true_swing_joint_target(robot, q_nom, q_release, phase, profile, swing_enabled)
