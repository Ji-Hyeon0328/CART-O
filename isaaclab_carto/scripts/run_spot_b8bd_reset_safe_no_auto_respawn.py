...
#
# B8-bd: reset-safe no-auto-respawn diagnostic.
#
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B8-bd reset-safe no-auto-respawn diagnostic")
...
def print_debug(step, phase, profile, phase_step, x_hat, base_ref, trial_out, ref_mask, contact_mask,
    ...
    print("\n" + "=" * 150)
    print(f"[B8-bd RESET-SAFE-NO-AUTO-RESPAWN] step={step}")
    print("=" * 150)
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B8-bd reset-safe no-auto-respawn diagnostic")
    ...
    for step in range(total_steps):
        ...
        ref_mask = all_stance_mask(args.num_envs, device, dtype) if phase in ["warmup", "settle", "recenter", "done"] or late_touchdown else future_swing_mask(args.num_envs, device, dtype)
