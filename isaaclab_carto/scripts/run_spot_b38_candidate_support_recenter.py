...
#
# B38: Candidate-specific support-region recenter before hind-leg liftoff.
#
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B38 candidate support recenter")
...
def apply_canonical_feasible_trunk_shift(base_ref, x_hat, foot_pos, exec_info, prev_base_ref, last_shift_target_xy=None):
    ...
    if args.canonical_enable_event_crawl_queue:
        ...
        if fm < float(args.canonical_event_shift_min_future_margin) and not b38_active_for_candidate:
    ...
    if args.canonical_shift_track_actual_base or (b38_active_for_candidate and args.b38_use_actual_base_start):
        start_xy = x_hat[:, 0:2]
    else:
    ...
    active_max_step = float(args.b38_max_step) if b38_active_for_candidate else float(args.canonical_shift_max_step)
...
def _b37_pick_leg_target_sigma(b23_info, b18_info):
    """Pick committed target and sigma, preferring B23 main foot ref."""
    if isinstance(b23_info, dict) and b23_info.get("active", False):
        leg_name = b23_info.get("leg")
        target = b23_info.get("committed", None)
        if target is None:
...
def print_debug(step, phase, profile, phase_step, x_hat, base_ref, trial_out, ref_mask, contact_mask,
    ...
    print("\n" + "=" * 150)
    print(f"[B38 CANDIDATE-SUPPORT-RECENTER] step={step}")
    print("=" * 150)
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B38 candidate-specific support-region recenter before hind-leg liftoff")
