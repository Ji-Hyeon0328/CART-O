# isaaclab_carto/scripts/run_spot_b8f_reduced_implicit_pd_probe.py
#
# B8-f: reduced implicit PD / mixed-control authority probe.
#
# Why:
#   B8-e showed that after the robot is settled, QPS-WBC residual torque
#   produces only tiny foot/joint motion because Isaac implicit actuator PD
#   is dominating the standing posture.
#
# Goal:
#   Reduce implicit actuator stiffness/damping at config level, then run the
#   same settled authority probe.
#
# This is still NOT a walking controller.
# It answers:
#   "If implicit PD is weaker, can QPS-WBC residual become visible without
#    immediately falling?"
#
# Important:
#   This script tries to patch actuator stiffness/damping generically because
#   Isaac Lab config structures differ between versions.
#
# Expected outcomes:
#   A) falls immediately:
#      PD reduction too aggressive; increase --pd_scale.
#   B) still stands unchanged:
#      residual still too weak or patch did not apply.
#   C) stands but shows small foot/leg motion:
#      good; residual authority is improving.
...
parser = argparse.ArgumentParser(description="CARTO/TRACER B8-f reduced implicit PD probe")
...
parser.add_argument("--target_scale", type=float, default=1.2)
...
parser.add_argument("--w_force", type=float, default=0.6)
parser.add_argument("--w_swing", type=float, default=0.60)
parser.add_argument("--w_posture", type=float, default=0.20)
parser.add_argument("--w_reg", type=float, default=0.02)
parser.add_argument("--w_rate", type=float, default=0.05)

parser.add_argument("--kp_swing_xy", type=float, default=35.0)
parser.add_argument("--kp_swing_z", type=float, default=60.0)
parser.add_argument("--kd_swing_xy", type=float, default=2.5)
parser.add_argument("--kd_swing_z", type=float, default=4.0)
parser.add_argument("--max_task_cmd", type=float, default=8.0)
parser.add_argument("--max_pos_err", type=float, default=0.08)

parser.add_argument("--kp_posture", type=float, default=0.7)
parser.add_argument("--kd_posture", type=float, default=0.04)
parser.add_argument("--max_posture_tau", type=float, default=0.20)

# Authority probe
parser.add_argument("--tau_probe_scale", type=float, default=10.0)
...
def patch_reduced_implicit_pd(env_cfg: Any) -> None:
    """
    Try to reduce actuator stiffness/damping.

    This is intentionally defensive because CartoEffortEnvCfg may store actuator
    configs differently depending on Isaac Lab version.
    """
    ...
    if isinstance(actuators, dict):
        items = actuators.items()
    else:
        items = [(str(i), a) for i, a in enumerate(actuators)]
...
def print_probe_debug(
    ...
    print("\n" + "=" * 132)
    print(f"[B8-f REDUCED IMPLICIT PD PROBE] probe_step={probe_step}")
    print("=" * 132)
    ...
    print("\n[authority probe after reduced implicit PD]")
    print("pd_scale:", args.pd_scale)
...
def main():
    ...
    print("\n" + "=" * 132)
    print("[INFO] Starting B8-f reduced implicit PD probe")
    print("=" * 132)
    print(f"pd_scale={args.pd_scale}, settle_steps={args.settle_steps}, probe_steps={args.probe_steps}, settle_mode={args.settle_mode}")
    print(f"z_mode={args.z_mode}, beta_preset={args.beta_preset}, tau_probe_scale={args.tau_probe_scale}")
    print("=" * 132 + "\n")

    # Phase 1: settle with reduced PD.
