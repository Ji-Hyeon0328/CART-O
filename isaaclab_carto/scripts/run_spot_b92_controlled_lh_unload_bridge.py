...
def _b92_controlled_unload_grf(f_mpc, x_hat, contact_mask, b90_info, unload_count=0, unload_candidate=None):
    ...
    if future_margin < float(getattr(args, "b92_min_future_margin", -0.085)):
        return f_mpc, {"active": False, "reason": "future_margin_too_low", "candidate_leg": cand, "future_margin": future_margin}, 0, None
    ...
    steps = max(1, int(getattr(args, "b92_unload_steps", 520)))
    min_alpha = float(getattr(args, "b92_min_unload_alpha", 0.60))
    ...
    removed_fz = (1.0 - alpha) * out[:, cand, 2:3].detach().clone()
    out[:, cand, 2:3] = alpha * out[:, cand, 2:3]
    ...
    release_alpha = float(getattr(args, "b92_release_alpha", 0.62))
    min_unload_steps = int(getattr(args, "b92_min_unload_steps", 420))
    release_ready = bool(new_count >= min_unload_steps and alpha <= release_alpha and future_margin >= float(getattr(args, "b92_min_future_margin", -0.085)))
...
def main():
    ...
    print("\n" + "=" * 150)
    print("[INFO] Starting B92 CONTROLLED LH/RH UNLOAD + B90/B89/B85 recovery planner")
    ...
    for step in range(total_steps):
        ...
        try:
            if int(b92_release_hold_count) > 0 and b92_release_leg is not None:
                ...
                b92_release_hold_count = max(1, int(getattr(args, "b92_hold_after_release_steps", 180)))
                contact_mask = all_stance_mask(args.num_envs, device, dtype)
