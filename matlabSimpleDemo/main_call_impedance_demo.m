%% main_call_impedance_demo.m
% Full prototype pipeline:
%   Theta Decoder -> Ref Mapper -> Force MPC -> Mock WBC -> Impedance Residual V1
%
% Version-1 residual:
%   Kp_eff = Kp_nom
%   Kd_eff = Kd_nom + touchdown_gate * alpha_imp * tanh(s_imp/s0) * I
%
% WBC and residual use mock Jacobians/foot states for interface verification.
...
%% Impedance residual parameters, Version 1
impParams = struct();
...
% Lookup table: conservative/aggressive x stance/swing
...
% Mock current foot error/velocity to make residual visible.
...
IMP = impedanceResidualV1(z_t, Theta, Ref, WBC, impParams);

%% Display
disp('===== tau_OC ====='); disp(WBC.tau_OC);
disp('===== tau_res ====='); disp(IMP.tau_res);
disp('===== tau_final ====='); disp(IMP.tau_final);
disp('===== impact_signal ====='); disp(IMP.impact_signal');
disp('===== phase_gate ====='); disp(IMP.phase_gate');

%% Visualization
figure;
imagesc(Ref.S); colormap(gray); colorbar;
...
title('WBC torque + Version-1 impedance residual'); grid on;
