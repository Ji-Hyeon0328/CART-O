%% main_go1_v3_diagnostics.m
% Go1-only diagnostic run for Version-3 impedance residual.
%
...
% This script uses:
%   - Go1 robot preset
%   - Force MPC with Go1 mass/inertia/hip offsets
%   - robot-scaled mock WBC, not true URDF dynamics yet
%   - Version-3 impedance residual
%
% Important:
%   wbcQP_robotScaledMock.m is still a mock WBC, but its mass matrix,
%   gravity bias, contact Jacobian scale, torque limit, and contact force
%   constraints are updated according to the selected robot preset.
...
a_HL = [
    0.20;   % swing clearance modulation
   -0.30;   % body height modulation
    0.40;   % duty factor modulation
   -0.10    % impedance modulation
];
...
% z will be overwritten by applyRobotPresetV3() to match Go1 nominal body height.
x_hat = [
    0.00; 0.00; 0.42;
    0.03; -0.02; 0.10;
    0.25; 0.02; 0.00;
...
%% ----------------------------
% Common parameters, initialized as toy and overwritten by preset
% -----------------------------
params.dt = 0.02;
params.N  = 20;
...
% overwritten by applyRobotPresetV3()
...
impParams.F0      = 60.0;  % overwritten by preset
...
disp("Raw force mismatch [N] = "); 
if isfield(IMP, "force_mismatch_raw")
    disp(IMP.force_mismatch_raw);
else
    disp("IMP.force_mismatch_raw field not found.");
end
...
title("Go1 contact schedule S_t, white=stance, black=swing");
colorbar;
...
legend("LF","RF","LH","RH");
title("Go1 optimized vertical GRF over horizon");
grid on;
...
legend("sum f_z", "mg");
title("Go1 total vertical GRF vs body weight");
grid on;
...
title("Go1 WBC torque + Version-3 impedance residual");
grid on;
...
title("Go1 torque limit usage ratio");
grid on;
yline(1.0, "--", "limit");
...
title("Go1 Version-3 residual signals");
grid on;
...
title("Go1 Version-3 gain adaptation summary");
grid on;
